...
variable "github_workflow_dispatch_token" {
  description = "Fine-grained PAT (permissão Actions: Read and write, escopado só a este repositório) usado pela EventBridge Connection para disparar lightsail_scheduler.yml via workflow_dispatch"
  type        = string
  sensitive   = true
}
