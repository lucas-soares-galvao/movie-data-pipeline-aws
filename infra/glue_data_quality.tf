...
resource "aws_glue_job" "data_quality_job" {
  ...
  command {
    ...
    script_location = "s3://${local.envs.s3_bucket_aux}/${local.tmdb_prefix}/${local.envs.glue_data_quality_job_name}/app/main.py"
    name            = "glueetl"
    python_version  = "3"
  }
...
resource "aws_s3_object" "deploy_scripts_bucket_data_quality" {
  ...
  key        = "${local.tmdb_prefix}/${local.envs.glue_data_quality_job_name}/app/main.py"
...
resource "aws_s3_object" "deploy_app_bundle_data_quality" {
  bucket     = aws_s3_bucket.auxiliary_bucket.id
  key        = "${local.tmdb_prefix}/${local.envs.glue_data_quality_job_name}/app_bundle.zip"
