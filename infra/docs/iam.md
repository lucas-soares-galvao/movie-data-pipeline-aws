...
| `cicd-terraform-observability-{env}` | EventBridge, CloudWatch (logs + alarms — inclui log groups `/lightsail/tmdb-*` e query definitions do Logs Insights, que exigem `Resource: "*"` pois o ID só existe após criação), SNS, SQS (DLQ) |
