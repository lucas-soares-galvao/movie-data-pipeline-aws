...
- Portas abertas: 22 (SSH — CIDR configurável via `lightsail_ssh_allowed_cidrs`), 80 (redirect HTTP→HTTPS + ACME challenge, público nos dois ambientes), 443 (HTTPS — proxy reverso para Streamlit). A 443 só é pública em **prod** (`local.https_public_cidrs` em `locals.tf`, `0.0.0.0/0` se `var.env == "prod"`, lista vazia em dev) — o `dynamic "port_info"` em `aws_lightsail_instance_public_ports.filmbot` não gera nenhum bloco para a 443 quando a lista está vazia, deixando a porta implicitamente fechada (modelo allow-list do firewall do Lightsail)
- **Acesso ao FilmBot de dev**: como a 443 fica fechada, o acesso é só via **Cloudflare Tunnel + Cloudflare Access** — `cloudflared` roda como serviço systemd na própria instância (instalado/registrado pelo step "Instalar/registrar Cloudflare Tunnel (dev)" de `04_deploy_lightsail.yml`, token vindo do GitHub Secret `AWS_CLOUDFLARE_TUNNEL_TOKEN_DEV`), abrindo uma conexão de saída para a Cloudflare e apontando para `https://localhost:443` (Caddy, via loopback — não passa pelo firewall do Lightsail). O domínio `filmbot-dev.lsgalvao.com.br` continua resolvendo, mas exige login (Cloudflare Access, e-mail autorizado) antes de qualquer tráfego chegar no servidor. Configuração do túnel/domínio/política de acesso é feita manualmente no painel da Cloudflare (fora do Terraform) — ver `C:\Users\lsgal\.claude\plans\verifique-em-deixar-o-adaptive-book.md` para o passo a passo. Prod não usa Cloudflare Tunnel — continua com a 443 pública normalmente
