...
FilmBot existe **só em prod** — dev não provisiona nenhum recurso Lightsail (instância, key pair, portas, IAM user do agente, static IP), controlado por `local.lightsail_prod_enabled` (`locals.tf`), que resolve `false` sempre que `var.env != "prod"`, independente do valor de `lightsail_enabled` no tfvars.
...
- IAM user `tmdb-filmbot-agent-prod` com acesso mínimo a Athena, S3 SPEC/TEMP, Glue Catalog e CloudWatch Logs
...
- Controlado pela variável `lightsail_enabled` — kill-switch manual dentro de prod (ex.: pausa emergencial de custo); sem efeito em dev, que nunca provisiona Lightsail independente desse valor. Quando habilitado, o workflow de deploy verifica o estado da instância via `aws lightsail get-instance` antes de tentar o SSH. Se a instância estiver destruída (fora da janela do scheduler), o deploy é **ignorado com warning** em vez de falhar por timeout.
...
- Como `lightsail_enabled` permanece `true` por padrão, qualquer `terraform apply` completo (não-targeted) disparado por um push normal em `main` recria a instância se ela estiver destruída no momento — ou seja, um deploy de código pode religar o servidor fora da janela agendada.
