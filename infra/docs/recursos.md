...
**Identidade de usuários — Cognito** (`lightsail_ia.tf`, mesmo gate `lightsail_agent_enabled` do IAM user do agente acima, dev e prod): `aws_cognito_user_pool.filmbot` guarda os usuários do FilmBot (login por e-mail, sem DynamoDB/RDS próprio — o pool já é o armazenamento persistente). `auto_verified_attributes` vazio de propósito: cadastro não dispara código, o gate de acesso é a aprovação manual do admin (`AdminConfirmSignUp`, ver `app/lightsail_ia/lightsail_ia.md`). `aws_cognito_user_pool_client.filmbot` sem client secret (chamadas só do backend) e `aws_cognito_user_group.admins` para o grupo de administradores. Sem `email_configuration` — usa o remetente nativo do próprio Cognito (sem SES) para o código de "esqueci a senha".

**Notificação de cadastro novo — SNS** (`sns_topics.tf`, tópico 10 de 10, mesmo padrão `protocol = "email"` dos outros 9): `filmbot_new_signup_notifications` avisa o admin por e-mail a cada `SignUp` novo no Cognito, publicado diretamente por `infrastructure.py::notify_new_signup()` via boto3 — sem EventBridge no meio, mesmo racional dos outros tópicos publicados diretamente por código Python (ex.: `backfill_success_notifications`).
