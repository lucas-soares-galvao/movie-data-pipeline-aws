...
def _run_main(
    ...
    with (
        ...
        patch("backfill_discover.shared.notify_backfill_success") as mock_notify,
    ):
...
class TestErros:
    ...
    def test_clienterror_que_nao_e_token_expirado_e_falha_soft_e_backfill_continua(self, monkeypatch):
        ...
        exc = ClientError({"Error": {"Code": "AccessDenied", "Message": "negado"}}, "PutObject")
        resultado: list = []
        mock_collect, *_ = _run_main(
            monkeypatch,
            {"BACKFILL_START_YEAR": "2020", "BACKFILL_END_YEAR": "2020"},
...
class TestNotificacaoSucesso:
    def test_chamado_uma_vez_quando_sem_falhas_e_trigger_agg_true(self, monkeypatch):
        ...
        mock_notify.assert_called_once_with(
            "discover",
            "Backfill de discover concluído sem pendências: 2 unidade(s) processada(s) "
