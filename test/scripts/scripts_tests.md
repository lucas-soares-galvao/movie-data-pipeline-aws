...
Sem gate de cobertura dedicado — `scripts/` não entra em `--cov=app`. Os testes rodam junto com o resto da suíte (`testpaths = test` em `pytest.ini`) e são **bloqueantes**: uma falha aqui reprova o step "Run tests with Coverage Gate" do CI (`.github/workflows/test.yml`) do mesmo jeito que uma falha em `app/`, só não conta para o percentual de cobertura exigido.
