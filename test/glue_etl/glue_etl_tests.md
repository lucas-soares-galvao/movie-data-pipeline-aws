...
- **`TestReadFromSorDiscover`** (8 testes): path S3 correto (`tmdb/discover/{media_type}/ano={year}/`) para movie e tv; coluna `year` adicionada ao DataFrame com valor correto; `overview_detected_language` calculado a partir de `overview` via `detect_fn`; `overview_translated_pt_br` — booleano puramente derivado de `overview_detected_language == "pt"`, **sem nenhuma tradução** (o `overview` já vem pt-BR nativo do TMDB via `lambda_api`) — `True`/`False` conforme o idioma detectado; guard de schema — sem a coluna `overview` (fixture mínima/legado), nem `overview_detected_language` nem `overview_translated_pt_br` são criadas
...
- **`TestReadFromSorWatchProvidersRef`** (5 testes): chave S3 correta para movie/tv; coluna `canonical_name` adicionada via `derive_canonical_name`; override aplicado (ex: "Paramount Plus" → "Paramount+"); ordem final das colunas bate com a declarada em `infra/glue_catalog.tf` (`provider_id, provider_name, display_priority_br, canonical_name, logo_path`) — o `ParquetHiveSerDe` resolve colunas por posição, então uma ordem diferente faz `canonical_name`/`logo_path` serem lidos trocados no Athena
...
- **`TestAddNamePtCountries`** (9 testes): traduz `english_name` para pt-BR via `translate_text` (mockada); sem coluna `english_name` retorna inalterado (nenhuma coluna nova, incluindo `name_detected_language_en`/`_pt`/`name_translation_attempts`); `english_name` vazio/nulo resulta em `name_pt` nulo; com `previous_df` e fonte idêntica reaproveita `name_pt` sem chamar `translate_text`; com fonte diferente traduz normalmente; `name_detected_language_en` calculado a partir de `english_name` (via `detect_fn` mockado); `name_detected_language_pt` é `"pt"` após tradução bem-sucedida; copia `english_name` direto para `name_pt` sem chamar tradução quando o idioma já é detectado como `"pt"` (mesma otimização do `glue_details` contra retradução infinita, impacto prático baixo aqui)
- **`TestAddNamePtLanguages`** (7 testes): traduz `english_name` dos idiomas para pt-BR via `translate_text` (mockada); sem coluna `english_name` retorna inalterado; `english_name` vazio/nulo resulta em `name_pt` nulo; com `previous_df` e fonte idêntica reaproveita `name_pt` sem chamar `translate_text` (detecção mockada — `langdetect` real não é confiável para nomes curtos como "Inglês"); com fonte diferente traduz normalmente; `name_detected_language_en` calculado a partir de `english_name`; copia direto sem chamar tradução quando idioma já detectado como `"pt"`
- **`TestReadFromSorConfigurationLanguages`** (2 testes): movie configuration recebe coluna `name_pt` traduzida via `translate_text` (mockada); `media_type` diferente de movie/tv devolve o DataFrame cru, sem `name_pt` e sem chamar a tradução
