...
- **`TestAddNamePtCountries`** (5 testes): traduz `english_name` para pt-BR via `translate_text` (mockada); sem coluna `english_name` retorna inalterado; `english_name` vazio/nulo resulta em `name_pt` nulo; com `previous_df` e fonte idêntica reaproveita `name_pt` sem chamar `translate_text`; com fonte diferente traduz normalmente
- **`TestAddNamePtLanguages`** (5 testes): traduz `english_name` dos idiomas para pt-BR via `translate_text` (mockada); sem coluna `english_name` retorna inalterado; `english_name` vazio/nulo resulta em `name_pt` nulo; com `previous_df` e fonte idêntica reaproveita `name_pt` sem chamar `translate_text`; com fonte diferente traduz normalmente
