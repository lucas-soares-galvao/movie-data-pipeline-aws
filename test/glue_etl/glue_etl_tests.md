...
- **`TestReadFromSorDiscover`** (7 testes): path S3 correto (`tmdb/discover/{media_type}/ano={year}/`) para movie e tv; coluna `year` adicionada ao DataFrame com valor correto; `overview_idioma_detectado` calculado a partir de `overview` via `detect_fn` — **só diagnóstico**, não há tradução no discover (o `overview` já vem pt-BR nativo do TMDB via `lambda_api`), então nenhuma coluna `overview_traduzido_pt_br` é criada; guard de schema — sem a coluna `overview` (fixture mínima/legado), `overview_idioma_detectado` não é criada
...
- **`TestAddNamePtCountries`** (9 testes): traduz `english_name` para pt-BR via `translate_text` (mockada); sem coluna `english_name` retorna inalterado (nenhuma coluna nova, incluindo `name_idioma_detectado`/`name_traduzido_pt_br`); `english_name` vazio/nulo resulta em `name_pt` nulo; com `previous_df` e fonte idêntica reaproveita `name_pt` sem chamar `translate_text`; com fonte diferente traduz normalmente; `name_idioma_detectado` calculado a partir de `english_name` (via `detect_fn` mockado); `name_traduzido_pt_br` é `True` após tradução bem-sucedida; copia `english_name` direto para `name_pt` sem chamar tradução quando o idioma já é detectado como `"pt"` (mesma otimização do `glue_details` contra retradução infinita, impacto prático baixo aqui)
- **`TestAddNamePtLanguages`** (7 testes): traduz `english_name` dos idiomas para pt-BR via `translate_text` (mockada); sem coluna `english_name` retorna inalterado; `english_name` vazio/nulo resulta em `name_pt` nulo; com `previous_df` e fonte idêntica reaproveita `name_pt` sem chamar `translate_text`; com fonte diferente traduz normalmente; `name_idioma_detectado` calculado a partir de `english_name`; copia direto sem chamar tradução quando idioma já detectado como `"pt"`
