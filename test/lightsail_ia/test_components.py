...
class TestRenderFeedback:
    def test_renderiza_classe_error(self, monkeypatch):
        ...
        components.render_feedback("error", "Algo deu errado.")
    ...
    def test_renderiza_classe_warning(self, monkeypatch):
        ...
        components.render_feedback("warning", "Fique atento.")
    ...
    def test_escapa_xss_na_mensagem(self, monkeypatch):
        ...
        components.render_feedback("error", '<script>alert("xss")</script>')
...
class TestLoadCountdownScript:
    def test_injeta_script_via_components_html(self, monkeypatch):
        ...
        components.load_countdown_script(42)
    ...
    def test_substitui_element_id_customizado(self, monkeypatch):
        ...
        components.load_countdown_script(42, element_id="audio-countdown")
...
class TestRenderPasswordRequirements:
    ...
    exibida abaixo do campo "Confirmar senha" nas telas de cadastro e redefinir senha."""

    def test_renderiza_lista_com_id_fixo(self, monkeypatch):
        captured = {}
        monkeypatch.setattr(
            components.st, "markdown",
            lambda content, unsafe_allow_html=False: captured.update(content=content),
...
class TestMatchesHighlighted:
    ...
    def test_sem_termos_retorna_falso(self):
        assert components._matches_highlighted("Terror", []) is False

    def test_item_bate_com_termo(self):
        assert components._matches_highlighted("Terror", ["terror"]) is True

    def test_item_nao_bate_com_termo(self):
        assert components._matches_highlighted("Drama", ["terror"]) is False
...
class TestParseProviderNames:
    ...
    def test_separa_nomes_por_virgula(self):
        nomes = components._parse_provider_names("Netflix,HBO Max")
        assert nomes == ["Netflix", "HBO Max"]

    def test_remove_espacos_em_volta_de_cada_nome(self):
        nomes = components._parse_provider_names("Netflix, HBO Max , Disney Plus")
        assert nomes == ["Netflix", "HBO Max", "Disney Plus"]

    def test_names_vazio_retorna_lista_vazia(self):
        assert components._parse_provider_names("") == []

    def test_none_retorna_lista_vazia(self):
        assert components._parse_provider_names(None) == []
...
class TestParseProviderLogos:
    ...
    def test_separa_urls_por_virgula(self):
        logos = components._parse_provider_logos("https://a.com/1.png,https://a.com/2.png")
    ...
    def test_preserva_entrada_vazia_no_meio(self):
        ...
        logos = components._parse_provider_logos("https://a.com/1.png,,https://a.com/3.png")
        assert logos == ["https://a.com/1.png", "", "https://a.com/3.png"]

    def test_logos_vazio_retorna_lista_de_um_item_vazio(self):
        assert components._parse_provider_logos("") == [""]

    def test_none_retorna_lista_de_um_item_vazio(self):
        assert components._parse_provider_logos(None) == [""]
...
class TestRenderProviderBadges:
    ...
    def test_renderiza_nome_como_texto_visivel(self):
        html = components._render_provider_badges([("Netflix", "")], [])
    ...
    def test_renderiza_logo_quando_disponivel(self):
        ...
        html = components._render_provider_badges([("Netflix", logo_url)], [])
    ...
    def test_escapa_html_no_nome(self):
        html = components._render_provider_badges([("<b>X</b>", "")], [])
        assert "<script>" not in html
        assert "&lt;b&gt;X&lt;/b&gt;" in html

    def test_escapa_html_na_url_da_logo(self):
        html = components._render_provider_badges(
    ...
    def test_corta_no_limite_de_provedores_visiveis(self):
        ...
        providers = [
            (f"Provedor{i}", "") for i in range(components._MAX_VISIBLE_PROVIDER_BADGES + 2)
        ]
        html = components._render_provider_badges(providers, [])
        assert "Provedor0" in html
        assert f"Provedor{components._MAX_VISIBLE_PROVIDER_BADGES}" not in html
...
class TestRenderCard:
    ...
    def test_card_sem_generos_nao_gera_genres_container(self):
        # Sem gêneros, a div nem é gerada (meio solto, ver cards.css) — mesmo padrão de
    ...
    def test_card_trailer_fica_na_meta_line_do_lado_esquerdo(self):
        ...
        html_com_poster = components.render_card(
        ...
        html_sem_poster = components.render_card(
    ...
    def test_card_sem_data_tipo_nota_trailer_nao_gera_meta_line(self):
        ...
        # nem é gerada (meio solto, ver cards.css) — mesmo padrão de
    ...
    def test_card_sem_duracao_nao_gera_duration_row(self):
        # Sem duração, a div nem é gerada (meio solto, ver cards.css).
        t = {**BASE_TITLE, "duration": None}
        html = components.render_card(t)
        assert "duration-row" not in html
    ...
    def test_card_toggle_id_usa_indice(self):
        html = components.render_card(BASE_TITLE, idx=3)
    ...
    def test_card_people_toggle_id_usa_indice(self):
        t = {**BASE_TITLE, "director": "Stanley Kubrick"}
        html = components.render_card(t, idx=3)
    ...
    def test_card_sem_sinopse_nem_trailer_nao_gera_row_synopsis(self):
        ...
        # cards.css) — mesmo padrão de row-people/duration-row/cinema-row.
...
class TestRenderCardSemPoster:
    ...
    def test_nao_gera_card_media(self):
        html = components.render_card(TITLE_SEM_POSTER)
...
class TestRenderGrid:
    def test_grid_vazio(self):
        html = components.render_grid([])
        assert "grid-titles" in html

    def test_grid_com_titulos(self):
        html = components.render_grid([BASE_TITLE, BASE_TITLE])
    ...
    def test_grid_gera_ids_de_people_toggle_unicos_por_indice(self):
        t = {**BASE_TITLE, "director": "Stanley Kubrick"}
        html = components.render_grid([t, t])
    ...
    def test_grid_nao_gera_posicionamento_inline_de_linha_coluna(self):
        ...
        # colunas (.grid-titles, ver cards.css) forma as fileiras sozinho via
