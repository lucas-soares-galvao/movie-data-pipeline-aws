...
| `test_skip_collect_details_when_discover_empty` | `collect_and_write_details` **não** é chamado quando o discover não retorna IDs para o ano |
| `test_skip_collect_watch_providers_when_discover_empty` | `collect_and_write_watch_providers` **não** é chamado quando o discover não retorna IDs para o ano |
...
### `TestExtractNextEpisodeToAir`

| Teste | O que verifica |
|---|---|
| `test_com_proximo_episodio` | Extrai `air_date`/`episode_number`/`season_number`/`name` do próximo episódio agendado |
| `test_chave_ausente` | Retorna todos os campos `None` quando `next_episode_to_air` não está no dict |
| `test_chave_none` | Retorna todos os campos `None` quando `next_episode_to_air` é `None` (série sem episódio futuro — a API retorna a chave com valor `None`, não ausente) |
| `test_campos_parciais` | Campos ausentes dentro do objeto (ex: só `air_date`) ficam `None` individualmente |

### `TestExtractSeasons`

| Teste | O que verifica |
|---|---|
| `test_lista_vazia` | Retorna todas as 4 colunas `None` quando `seasons` está ausente/vazia |
| `test_uma_temporada` | Extrai número, data, qtd. de episódios e nome de uma única temporada |
| `test_multiplas_temporadas` | Colunas comma-separated alinhadas posicionalmente entre múltiplas temporadas |
| `test_temporada_futura_sem_data_preserva_alinhamento_posicional` | Temporada com `air_date`/`episode_count` `None` (anunciada, ainda sem data) gera posição vazia na string em vez de ser filtrada, mantendo `season_numbers`/`season_air_dates`/`season_episode_counts`/`season_names` alinhados índice a índice |
...
| `TestResolveMatchedIdsForChangedIds.test_nao_grava_arquivo_de_descartados_quando_todos_encontrados` | Nenhum arquivo de descartados é gravado quando todos os IDs têm match |
| `TestResolveMatchedIdsForChangedIds.test_faz_chunking_da_clausula_in` | A consulta Athena (`GROUP BY id`) é feita em lotes de `chunk_size` |
| `TestResolveMatchedIdsForChangedIds.test_grava_ids_ambiguos_quando_id_tem_mais_de_um_year_no_discover` | Id com `year_count > 1` (mais de um year na discover) é logado em `discover_years_ambiguos_{data}.json` |
| `TestResolveMatchedIdsForChangedIds.test_nao_grava_arquivo_de_ambiguos_quando_nenhum_id_tem_mais_de_um_year` | Nenhum arquivo de ambíguos é gravado quando todo id tem `year_count == 1` |
| `TestResolveMatchedIdsForChangedIds.test_id_ambiguo_ainda_aparece_uma_unica_vez_na_lista_de_matched_ids` | Um id com múltiplos years na discover aparece só uma vez em `matched_ids` — a ambiguidade não duplica nem descarta o id |
| `TestProcessChangedIds.test_retorna_lista_vazia_se_nenhum_id_encontrado` | Retorna `[]` sem chamar enriquecimento quando nenhum ID é confirmado no catálogo |
| `TestProcessChangedIds.test_chama_collect_and_write_details_com_todos_os_ids_resolvidos` | `collect_and_write_details` recebe todos os IDs confirmados, sem `year` |
| `TestProcessChangedIds.test_agrupa_watch_providers_por_year` | `collect_and_write_watch_providers` é chamado uma vez por ano distinto (year vindo de `collect_and_write_details`), com os IDs daquele ano |
...
| `TestProcessChangedIds.test_watch_providers_usa_year_retornado_por_collect_and_write_details_nao_do_discover` | O `year` passado a `collect_and_write_watch_providers` vem do dict retornado por `collect_and_write_details` (API), não da discover |
