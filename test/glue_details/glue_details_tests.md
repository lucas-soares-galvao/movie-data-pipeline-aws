...
- `_add_translations_pt` (`TestAddTranslationsOverviewPt`): detecta `overview_idioma_detectado` a partir de `overview_en` (via `detect_fn` mockado, nunca a partir de `overview_pt`), prioriza tradução pt-BR do TMDB, fallback para Google Translate quando TMDB não tem, traduz mesmo quando `original_language` já é `pt` (não é critério de elegibilidade), loga resumo "N registros traduzidos com sucesso (overview_pt)" em INFO, retenta quando `overview_pt_tmdb` fica igual a `overview_en`, grava `overview_traduzido_pt_br` (`True` para nativo TMDB/sucesso Google, `False` para falha/fonte vazia), e copia `overview_en` direto para `overview_pt` sem chamar tradução quando o idioma já é detectado como `"pt"` (otimização contra retradução infinita)
- `_add_translations_keywords_pt` (`TestAddTranslationsKeywordsPt`): detecta `keywords_idioma_detectado`, traduz via Google Translate, traduz mesmo quando `original_language` já é `pt` (TMDB não localiza keywords), não traduz quando `keywords` vazia (`keywords_traduzido_pt_br` fica `False`), copia direto sem chamar tradução quando idioma já detectado como `"pt"`
- `_add_translations_tagline_pt` (`TestAddTranslationsTaglinePt`): detecta `tagline_idioma_detectado`, prioriza tradução pt-BR do TMDB, fallback para Google Translate quando TMDB não tem, ignora vazia/nula, traduz mesmo quando `original_language` já é `pt` (não é critério de elegibilidade), retenta quando `tagline_pt_tmdb` fica igual a `tagline`, copia direto sem chamar tradução quando idioma já detectado como `"pt"`
...
| `test_idioma_detectado_calculado_a_partir_da_fonte` | `overview_idioma_detectado` chama `detect_fn` com `overview_en` (a fonte), nunca com `overview_pt` |
| `test_idioma_detectado_none_quando_fonte_vazia` | `overview_en` vazio/nulo → `overview_idioma_detectado` fica nulo |
| `test_traduzido_pt_br_true_para_nativo_tmdb` | `overview_traduzido_pt_br` é `True` quando `overview_pt_tmdb` preencheu `overview_pt` |
| `test_traduzido_pt_br_true_para_sucesso_google` | `overview_traduzido_pt_br` é `True` após tradução via Google bem-sucedida |
| `test_traduzido_pt_br_false_quando_traducao_falha` | `overview_traduzido_pt_br` é `False` quando a tradução falha e mantém o texto original |
| `test_traduzido_pt_br_false_quando_fonte_vazia` | `overview_traduzido_pt_br` é `False` quando não há `overview_en` |
| `test_copia_direta_quando_fonte_ja_detectada_como_pt_sem_chamar_traducao` | Fonte com idioma detectado `"pt"` (sem TMDB nativo/cache) é copiada direto para `overview_pt`; `translate_fn` mockado **não** é chamado; `overview_traduzido_pt_br` fica `True` |
...
| `test_nao_traduz_quando_keywords_vazias` | `keywords_pt` fica nulo quando `keywords` está vazia/nula; `keywords_traduzido_pt_br` fica `False` |
| `test_idioma_detectado_calculado_a_partir_da_fonte` | `keywords_idioma_detectado` chama `detect_fn` com `keywords` (a fonte) |
| `test_copia_direta_quando_fonte_ja_detectada_como_pt_sem_chamar_traducao` | Fonte com idioma detectado `"pt"` é copiada direto para `keywords_pt`, sem chamar `translate_fn`; `keywords_traduzido_pt_br` fica `True` |
...
| `test_nao_traduz_quando_tudo_vazio` | `tagline`/`tagline_pt_tmdb` vazios → `tagline_pt` fica nulo, `tagline_traduzido_pt_br` fica `False` para todos os registros |
...
| `test_copia_direta_quando_fonte_ja_detectada_como_pt_sem_chamar_traducao` | Fonte com idioma detectado `"pt"` é copiada direto para `tagline_pt`, sem chamar `translate_fn`; `tagline_traduzido_pt_br` fica `True` |
