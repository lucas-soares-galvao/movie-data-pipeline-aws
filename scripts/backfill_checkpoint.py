...
Se o script for interrompido (ex.: ExpiredTokenException) e reiniciado com o
mesmo table_group e o mesmo range de anos, ele pula direto para as unidades
ainda não concluídas em vez de recomeçar do start_year.
"""
...
def expired_token_exit_code(exc: ClientError) -> int | None:
    ...
    ExpiredTokenException é um erro retomável (o workflow renova a credencial
    ...
    Returns:
        `RETRYABLE_EXIT_CODE` (75) se for ExpiredTokenException, `None` caso
        contrário (o chamador deve relançar a exceção original).
    """
...
def _log_expired_token(exc: ClientError, contexto: str) -> None:
    """Loga um erro claro se a credencial AWS expirou durante o acesso ao checkpoint."""
    if exc.response.get("Error", {}).get("Code") == "ExpiredTokenException":
        logger.error(
...
def load_checkpoint(
    ...
    try:
        ...
        if codigo in ("NoSuchKey", "404"):
            return set()
        _log_expired_token(exc, f"leitura do checkpoint '{key}'")
        raise
...
def save_checkpoint(
    ...
    try:
        ...
        _log_expired_token(exc, f"escrita do checkpoint '{key}'")
        raise
...
def clear_checkpoint(s3_client: Any, bucket: str, table_group: str) -> None:
    ...
    try:
        ...
        _log_expired_token(exc, f"remoção do checkpoint '{key}'")
        raise
