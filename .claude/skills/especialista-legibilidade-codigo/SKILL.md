...
- **SQL como string fixa com placeholders (`.format()`), nunca concatenação dinâmica de fragmentos** — é o padrão em todo o projeto. A única exceção controlada é `app/lightsail_ia/agent.py`, onde uma cláusula `WHERE` gerada por LLM passa por `_validate_where` antes de ser interpolada (mecanismo em `especialista-engenharia-dados-app`, racional de segurança/SQL injection em `especialista-seguranca-filmbot`); replicar esse padrão de validação para qualquer SQL novo montado a partir de input externo.
