...
| `05_lightsail_scheduler.yml` | `workflow_dispatch` isolado (input `action`: choice `start`/`stop`) — chama `04_deploy_lightsail.yml` via `uses:` no job `deploy-app`. Sem `schedule:` nativo — o agendamento diário (liga 08:00 BRT / desliga 00:00 BRT) é acionado pela AWS via duas `aws_cloudwatch_event_rule` (`infra/lightsail_scheduler_trigger.tf`) chamando a API REST do GitHub (`workflow_dispatch`) através de uma EventBridge API Destination, evitando o atraso documentado do `schedule:` do GitHub Actions em horário de pico | `id-token: write`, `contents: read` |
...
|---|---|---|---|
| `01_test.yml` | — | — | — |
| `02_terraform.yml` | `environment` (string) | `aws-assume-role-arn`, `aws-statefile-s3-bucket`, `aws-lock-dynamodb-table`, `aws-filmbot-secret-arn`, `aws-github-workflow-dispatch-token`, `notification-email`, `infracost-api-key` (7) | `was_destroyed` (= `jobs.terraform.outputs.destroyed_status`) |
...
`05_lightsail_scheduler.yml` resolve o ambiente de outro jeito (não tem `push` por branch para inferir dev/prod, e é `workflow_dispatch` isolado — sem `schedule:` próprio, ver tabela de triggers acima): um step `resolve` dedicado decide `environment` a partir do branch (`main` → `prod`, qualquer outro falha) e `action` direto de `inputs.action`, grava em `$GITHUB_OUTPUT`, e um step `secrets` seguinte lê os 5 secrets `_PROD` (sem seleção `_DEV`/`_PROD`, já que o workflow só existe para prod) via `env:`/`echo >> $GITHUB_OUTPUT`. O job `deploy-app` (que chama `04_deploy_lightsail.yml`) usa a ternária inline do YAML, igual às de `00_pipeline.yml`, mas sempre resolvendo para `_PROD` na prática.
...
- Dentro de `04_deploy_lightsail.yml`: 5 steps encadeados em `if: steps.tf.outputs.lightsail_enabled == 'true' && steps.instance_check.outputs.instance_running == 'true'` (setup SSH, criar `.env`, criar `.env.caddy`, deploy, health check)
- `deploy-app` (em `05_lightsail_scheduler.yml`): `needs.lightsail-schedule.outputs.action == 'start' && needs.lightsail-schedule.result == 'success'` — só reidrata a instância quando ela acabou de ser criada, nunca depois de um `stop`
- Dentro de `05_lightsail_scheduler.yml` (mesmo job `lightsail-schedule`): destroy e apply mutuamente exclusivos via `if: steps.resolve.outputs.action == 'stop'` / `== 'start'` — mesmo padrão do apply/destroy mutuamente exclusivo de `02_terraform.yml`, mas decidido por um step `resolve` próprio em vez de um arquivo de config (`destroy_config.json`)
...
- Em `04_deploy_lightsail.yml`, o step `tf` (id `tf`) produz ~10 outputs consumidos por 5 steps seguintes: `public_ip`, `lightsail_enabled`, `instance_name`, `key_file`, `access_key_id` (mascarado via `::add-mask::`), `secret_access_key` (mascarado), `cw_log_group`, `filmbot_secret_arn` (mascarado), `athena_s3_output`, `glue_database`, `spec_table`
- Em `06_backfill.yml`, o step `env` (id `env`) produz `name` (dev/prod, resolvido do branch) e o step `project` produz `project_prefix` (lido de `infra/config/project.json` via `jq`) — ambos interpolados em ~20 variáveis de ambiente do step "Run backfill" (nomes de job Glue, databases, buckets, tabelas)
- Em `05_lightsail_scheduler.yml`: `resolve.outputs.{environment,action}` → consumidos por `secrets.outputs.*` (mesmo step order) e por todos os steps de terraform seguintes, e expostos como `outputs.{environment,action}` do job `lightsail-schedule` → consumidos pelo `if:`/`with: environment:`/seleção de secrets do job `deploy-app`

## Segurança de supply-chain — o achado mais importante

- **Nenhuma ação de terceiro é pinada por SHA** — todas por tag major mutável: `actions/checkout@v4` (`01_test.yml`, `02_terraform.yml`, `03_pr_auto.yml`, `04_deploy_lightsail.yml`, `05_lightsail_scheduler.yml`, `06_backfill.yml` — todo workflow que faz checkout), `actions/setup-python@v5` (`01_test.yml`, `06_backfill.yml`), `hashicorp/setup-terraform@v3` (`02_terraform.yml`, `03_pr_auto.yml`, `04_deploy_lightsail.yml`, `05_lightsail_scheduler.yml`), `aws-actions/configure-aws-credentials@v4` (`02_terraform.yml`, `04_deploy_lightsail.yml`, `05_lightsail_scheduler.yml`, `06_backfill.yml`). Uma tag `@v4` re-apontada por um ataque de supply-chain entregaria código malicioso já de posse de credenciais AWS via OIDC — `configure-aws-credentials` é a ação mais sensível do conjunto.
...
- **`concurrency:`** existe em `02_terraform.yml` e `05_lightsail_scheduler.yml`, a nível de job. Em `02_terraform.yml`: `group: terraform-${{ inputs.environment }}`, `cancel-in-progress: false`. Em `05_lightsail_scheduler.yml`: mesmo prefixo de group (`terraform-<ambiente>`), mas o ambiente é resolvido inline na expressão do próprio `group:` (`github.event_name == 'workflow_dispatch' && inputs.environment || 'prod'`, já que não há `needs`/step anterior disponível nesse ponto do YAML) — **deliberadamente o mesmo nome de group** que `02_terraform.yml` usa para aquele ambiente, para serializar o scheduler com qualquer apply/destroy completo disparado por push (evita dois processos escrevendo no mesmo state ao mesmo tempo). Nenhum outro workflow usa `concurrency:`, nem `00_pipeline.yml`. Isso não cobre cancelamento manual de um run em andamento — cancelar mata o processo do terraform sem ele liberar o lock do DynamoDB, mesmo sem nenhuma execução concorrente real.
- **Force-unlock automático em cancelamento**: último step de `02_terraform.yml` e de `05_lightsail_scheduler.yml` (`if: cancelled()`, depois do Apply/Destroy) — consulta o item de lock direto na tabela DynamoDB, extrai o `ID` de dentro do atributo `Info` e roda `terraform force-unlock -force <ID>` (nunca ID hardcoded); vira no-op se não houver lock (job cancelado antes de qualquer terraform rodar). Únicos steps do repositório que reagem a `cancelled()` em vez do padrão implícito `success()`.
- **`defaults.run.shell: bash`** declarado em `02_terraform.yml`, `04_deploy_lightsail.yml` e `05_lightsail_scheduler.yml` — ativa `pipefail`. É por isso que `02_terraform.yml` usa `terraform state show <addr>` em vez de `terraform state list | grep`: sob `pipefail`, se o `grep` fecha o pipe antes do `terraform` terminar de imprimir o state inteiro, o `terraform` recebe `EPIPE` e o `if` cai no `else` mesmo com o `grep` tendo funcionado — ver comentário no próprio step "Import existing CI/CD role". Os outros 2 workflows não declaram `defaults.run.shell`, então não têm `pipefail` ativo por padrão.
- **Cache**: só `01_test.yml` usa (`actions/setup-python@v5` com `cache: "pip"` e `cache-dependency-path` apontando para `app/**/requirements.txt` + `test/**/requirements_tests.txt`). `06_backfill.yml` chama `actions/setup-python@v5` **sem** `cache:` — inconsistência a observar em review, não necessariamente um problema (backfill roda raramente).
...
- **`timeout-minutes`** só declarado em 2 jobs: `01_test.yml` (`15`) e `06_backfill.yml` (`360`, deliberadamente alinhado ao `max_tentativas=6` do loop de retry a ~1h por sessão AWS — se um mudar, o outro precisa mudar junto, ver comentário no step "Run backfill"). `05_lightsail_scheduler.yml` não declara — um `terraform apply/destroy -target` travado (ex.: throttling da API Lightsail) só é limitado pelo timeout padrão do runner (360min).
- **Sem composite actions** (`.github/actions/` não existe) — todo reuso entre os 7 arquivos é via `uses: ./.github/workflows/X.yml` (reusable *workflows*, não composite *actions*). Lógica duplicada e não fatorada: leitura de `infra/config/project.json` via `jq` (`02_terraform.yml`, `04_deploy_lightsail.yml`, `05_lightsail_scheduler.yml`, parcialmente `06_backfill.yml`), setup do `hashicorp/setup-terraform` com `terraform_wrapper: false` (`02_terraform.yml`, `04_deploy_lightsail.yml`, `05_lightsail_scheduler.yml`), a ternária de seleção de secret por ambiente (seção acima).
