...
| `05_lightsail_scheduler.yml` | `schedule` (3 crons, só prod) + `workflow_dispatch` (inputs `environment`: choice `prod`/`dev`, `action`: choice `start`/`stop`) — chama `04_deploy_lightsail.yml` via `uses:` no job `deploy-app` | `id-token: write`, `contents: read` |
...
`05_lightsail_scheduler.yml` resolve o ambiente de outro jeito (não tem `push` por branch para inferir dev/prod): um step `resolve` dedicado decide `environment`/`action` a partir do trigger (`workflow_dispatch` usa `inputs.*` diretamente; `schedule` sempre resolve `prod`, com `action` decidido comparando `github.event.schedule` contra a string exata do cron de "desligar"), grava em `$GITHUB_OUTPUT`, e um step `secrets` seguinte faz a seleção `_DEV`/`_PROD` via `if` bash (não ternária inline do YAML) lendo de `env:` — mesma necessidade (secret não pode ser montado por interpolação de nome), mecanismo diferente do padrão ternário de `00_pipeline.yml`. O job `deploy-app` (que chama `04_deploy_lightsail.yml`) volta a usar a ternária inline do YAML, iguais às de `00_pipeline.yml`.
