...
│   ├── iam_cicd.tf                 # policies least-privilege da role GitHub Actions (7 em prod, 6 em dev) + sync
...
│   ├── lightsail_ia.tf             # Instância Lightsail + IAM user filmbot-agent (só prod)
...
push em develop    →  test  →  terraform (dev)  →  deploy-lightsail sempre "skipped" (FilmBot não existe em dev) + auto-pr-environment (develop → main), em paralelo — os dois só dependem de `terraform`, não um do outro
push em main       →  test  →  terraform (prod)  →  deploy-lightsail (prod, se a instância estiver ligada)
                          →  sonar (SonarQube Cloud, reaproveita o coverage.xml do test), em paralelo ao terraform — nenhum dos dois depende do outro, ambos esperam só `test`
workflow_dispatch  →  test (se a ref for feature/*|develop|main)  →  terraform (env escolhido)  →  deploy-lightsail só roda se o ambiente escolhido for prod
```

**Job `sonar`:** `if: github.ref_name == 'main' && needs.test.result == 'success'`, `needs: [resolve-env, test]`. O `needs: resolve-env` é só organizacional/visual (agrupa no grafo da Actions ao lado de `terraform`/`deploy-lightsail`): `sonar` não consome nenhum output de `resolve-env`. O `needs: test` é real: o `sonar.yml` não roda pytest, baixa o `coverage.xml` que o `test.yml` publica como artifact `coverage-xml` (só em `main`), e não faz sentido analisar um commit que o gate já reprovou. Roda em paralelo ao `terraform` (nenhum `needs` entre os dois), já que um provisiona infra e o outro analisa código-fonte, sem dependência real entre eles — ambos esperam só `test`, então o Sonar não atrasa o deploy. Chama `sonar.yml` via `uses:`, repassando `SONAR_TOKEN` como o secret `sonar-token` (mesma mecânica de repasse que `terraform`/`deploy-lightsail` já usam para os secrets AWS). Só roda em `push:main` (não em `develop` nem `feature/*`) porque o plano Free do SonarQube Cloud só analisa de forma contínua a branch marcada como "principal" no projeto Sonar — tentar analisar qualquer outra branch (`develop`, por exemplo) é rejeitado/ignorado pelo Sonar sem erro visível no CI (a branch aparece como "Not analyzed" no dashboard, atrás de um selo "Upgrade"). Não decora Pull Requests (decisão consciente: exigiria adicionar o evento `pull_request` ao trigger deste orquestrador, o que faria os jobs `terraform`/`deploy-lightsail` também serem avaliados nesse evento).
...
13. **Bootstrap das IAM policies do CI/CD** (se não for destroy) — `terraform apply -target` nas policies/attachments (7 em prod, 6 em dev — `cicd_lightsail` só existe em prod), depois faz polling em `aws iam list-attached-role-policies` (a cada 5s, timeout 60s) até confirmar que todas estão attachadas
...
**Branch deployada:** sempre `main` (fixo no workflow — só prod chama este job)
...
| `iam_cicd.tf` | Policies least-privilege da role GitHub Actions — 7 em prod, 6 em dev (`cicd_lightsail` só existe em prod) — (nome/prefixo lidos de `infra/config/project.json`, default `lsg-github-actions-{env}`) + `terraform_data` de sincronização |
...
| `lightsail_ia.tf` | Instância Lightsail + IAM user filmbot-agent — só prod (`local.lightsail_prod_enabled`, ver `recursos.md`) |
