...
**Etapas:** Checkout → resolve ambiente pelo branch → lê `infra/config/project.json` (`project_prefix`, step "Read project configuration") → autenticação OIDC com `AWS_ASSUME_ROLE_ARN_BACKFILL_DEV` ou `AWS_ASSUME_ROLE_ARN_BACKFILL_PROD` conforme o ambiente resolvido (sessão padrão de 1h) → Setup Python 3.12 → instala `boto3` (+ `scripts/requirements_backfill.txt` só para `traducao`/`rename_colunas`) → executa o script correspondente dentro de um loop de retry (até 6 tentativas, renovando a credencial via OIDC a cada exit code 75) → ao terminar com sucesso, dispara `glue:StartJobRun` no `glue_agg` (fire-and-forget) para todo `table_group` exceto `data_quality`, propagando o backfill à camada SPEC sem esperar o próximo ciclo agendado (ver `especialista-scripts-backfill`, `app/glue_agg/glue_agg.md`). `timeout-minutes: 360`.
