...
- **Lightsail** (`lightsail_ia.tf`, `lightsail_scheduler.tf`): `bundle_id="micro_3_0"` é o menor bundle pago (~$7/mês fixo, cobrado independente de uso). O scheduler (Lambda + 3 regras EventBridge) desliga a instância todo dia às 00:00 BRT e liga só às 18:00 BRT (dias úteis) ou 08:00 BRT (fins de semana) — reduz o tempo ligado em ~40h/semana. `lightsail_enabled=false` em dev elimina esse custo por completo nesse ambiente.
