...
- **Glue Catalog restrito por `database`/`table` ARN específico por role**: `glue_agg_catalog` é a única policy com `glue:DeleteTable`, e mesmo essa está restrita a `database/unified` + `table/unified/*` — coerente com o AGG sendo o único job que recria a tabela SPEC.
