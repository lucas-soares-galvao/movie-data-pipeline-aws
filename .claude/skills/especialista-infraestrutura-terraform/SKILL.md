...
- Único padrão de repetição no projeto é `count = local.lightsail_prod_enabled ? 1 : 0` — não há `for_each` em lugar nenhum de `infra/`.
...
A VM existe **só em prod** — `aws_lightsail_instance.filmbot`, key pair, IP estático + attachment e portas públicas são gated por `local.lightsail_prod_enabled` (`locals.tf`), que resolve `false` sempre que `var.env != "prod"`, independente do valor de `var.lightsail_instance_enabled` no tfvars — `bundle_id=var.lightsail_bundle_id` (`micro_3_0`), `blueprint_id="ubuntu_22_04"`, AZ `us-east-1a`, `provider = aws.lightsail`. Portas: 22 restrita a `var.lightsail_ssh_allowed_cidrs`; 80/443 abertas a `0.0.0.0/0`. Já o IAM user do agente é gated separadamente por `local.lightsail_agent_enabled` (`= var.lightsail_agent_enabled`, sem restrição de env) — existe em dev e prod, pois não tem custo.

Os outputs que indexam `recurso[0]` (`lightsail_public_ip`, `lightsail_url`, `lightsail_private_key`, `lightsail_instance_name`, `lightsail_agent_access_key_id`, `lightsail_agent_secret_access_key`, `lightsail_cloudwatch_log_group`) usam guarda `length(recurso) > 0 ? recurso[0].attr : ""`, não `local.lightsail_prod_enabled ? ... : ""` — necessário porque o scheduler (`lightsail_scheduler.yml`) destrói/recria `aws_lightsail_instance.filmbot` via `-target` enquanto `lightsail_instance_enabled` permanece `true`; com a guarda antiga, `terraform destroy -target=aws_lightsail_instance.filmbot` quebraria ao recalcular os outputs no final (índice inválido numa lista vazia).

Usa `aws_iam_user.lightsail_agent` (**usuário, não role** — a instância Lightsail não assume roles IAM) com access key gerenciada pelo Terraform (output sensível), policy escopada para Athena, S3 (SPEC leitura, TEMP leitura/escrita), Glue read, CloudWatch Logs, Secrets Manager (`aws_iam_user_policy.filmbot_secrets_manager` em `iam_policies.tf`, mesmo gate). Vários `output` (IP, URL, private key, access keys, log group, path de saída do Athena, DB/tabela do Glue) alimentam o `.env` da instância — consumidos pelo workflow `deploy_lightsail.yml`, não fixados no Terraform.
...
REST do GitHub (`workflow_dispatch` em `lightsail_scheduler.yml`) via uma `aws_cloudwatch_event_api_destination`.
...
`terraform.yml` quanto em `lightsail_scheduler.yml` (Terraform exige todas as variáveis sem default mesmo em
...
- **`iam_cicd.tf`** — `aws_iam_role.github_actions` (`lsg-github-actions-{env}`, trust OIDC, `max_session_duration=3600`) + policies least-privilege (backend/state-lock, S3, IAM self-mgmt escopado a `tmdb-*`, compute, observability, ssm) — 7 em prod, 6 em dev, já que `cicd_lightsail` (Instance/KeyPair/StaticIp) só existe em prod (`count = lower(var.env) == "prod" ? 1 : 0`, mesmo motivo do Lightsail em si não existir em dev). Termina em `terraform_data.cicd_policies_ready` — **quase todo outro recurso do projeto depende dela** — seguida de `null_resource.cicd_policies_propagation` que faz polling em `iam:SimulatePrincipalPolicy` (até 60s) contornando a consistência eventual do IAM.
- **`iam_backfill.tf`** — role separada `tmdb-backfill-role-{env}` para o workflow manual `backfill.yml` (antes reusava a role de CI/CD — excesso de privilégio). Trust policy restrita por repo **e** branch (`local.backfill_allowed_branch`: dev→`develop`, prod→`main`). Policies escopadas: invocar Lambda, iniciar/monitorar Glue Details+DQ, S3 (checkpoints + prefixos específicos da SOT), Glue Catalog (só tabelas de details/watch_providers).
...
`var.env` (`dev`/`prod`, **contas AWS separadas** via AWS Organizations) controla, direta ou indiretamente: `local.lightsail_prod_enabled` (VM/DNS do Lightsail só existe em prod, independente de `var.lightsail_instance_enabled` — o IAM user do agente, via `local.lightsail_agent_enabled`, existe em ambos), `var.log_retention_days`, `local.eventbridge_schedule_state` (schedules só ativos em prod), `var.filmbot_secret_arn` e todos os e-mails de notificação por tópico — todos vêm de `envs/{dev,prod}/terraform.tfvars`. `cicd_statefile_s3_bucket`/`cicd_lock_dynamodb_table` **não** ficam em tfvars — chegam via `-var` a partir de secrets do GitHub Actions no workflow `terraform.yml`.
