...
FilmBot existe **só em prod** — `aws_lightsail_instance.filmbot` e todos os demais recursos do arquivo (key pair, IP estático + attachment, portas públicas, IAM user do agente) são gated por `local.lightsail_prod_enabled` (`locals.tf`), que resolve `false` sempre que `var.env != "prod"`, independente do valor de `var.lightsail_enabled` no tfvars — `bundle_id=var.lightsail_bundle_id` (`micro_3_0`), `blueprint_id="ubuntu_22_04"`, AZ `us-east-1a`, `provider = aws.lightsail`. Portas: 22 restrita a `var.lightsail_ssh_allowed_cidrs`; 80/443 abertas a `0.0.0.0/0`.
...
`var.env` (`dev`/`prod`, **contas AWS separadas** via AWS Organizations) controla, direta ou indiretamente: `local.lightsail_prod_enabled` (Lightsail só existe em prod, independente de `var.lightsail_enabled`), `var.log_retention_days`, `local.eventbridge_schedule_state` (schedules só ativos em prod), `var.filmbot_secret_arn` e todos os e-mails de notificação por tópico — todos vêm de `envs/{dev,prod}/terraform.tfvars`. `cicd_statefile_s3_bucket`/`cicd_lock_dynamodb_table` **não** ficam em tfvars — chegam via `-var` a partir de secrets do GitHub Actions no workflow `02_terraform.yml`.
