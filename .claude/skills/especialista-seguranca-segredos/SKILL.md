...
- **`deploy_lightsail.yml`**: a chave privada SSH do Lightsail é lida via `terraform output -raw` para um arquivo criado com `mktemp` (nunca um caminho fixo em `/tmp` mundialmente legível) e recebe `chmod 600` antes de qualquer uso. Access key, secret key e o ARN do secret do FilmBot passam por `::add-mask::` **antes** de aparecerem em qualquer output subsequente do job, impedindo que apareçam em texto puro no log do Actions. O `.env` de produção é escrito diretamente no destino via `printf | ssh | tee` — nunca passa pelo workspace do runner.
- **`github_workflow_dispatch_token` (`lightsail_scheduler_trigger.tf`) é o único segredo do projeto que flui na direção contrária** (GitHub → AWS, não AWS → GitHub via OIDC): um fine-grained PAT do GitHub armazenado como GitHub Secret (`AWS_GH_WORKFLOW_DISPATCH_TOKEN_{DEV,PROD}`), passado via `-var` (nunca `terraform.tfvars`, mesmo padrão de `filmbot_secret_arn`) para autenticar a `aws_cloudwatch_event_connection` que dispara `workflow_dispatch` do GitHub a partir do EventBridge. A própria AWS re-armazena o valor no Secrets Manager ao criar a connection — mesmo perfil de risco já aceito para `lightsail_private_key` (sensível, presente no tfstate).
...
- Credencial nova exposta num step de workflow: aplicar `::add-mask::` antes de qualquer `echo`/output subsequente, seguindo o padrão de `deploy_lightsail.yml`.
