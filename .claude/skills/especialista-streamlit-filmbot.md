...
- **Grid responsivo mobile-first**: `grid-template-columns: repeat(auto-fill, minmax(Xpx, 1fr))` como padrão para grids de cards, ajustando o `minmax` por breakpoint — `260px` no desktop, `200px` no tablet, coluna única (`1fr`) no mobile estreito. Replique o padrão de `.grid-titles` em `principal.css`.
- **Breakpoints já estabelecidos no projeto**: `1024px` (tablet/mobile largo) e `480px` (mobile estreito) — reutilize esses dois antes de inventar um novo.
...
- Se a mudança tocar `utils.py` ou lógica Python testável, siga o checklist padrão do projeto (skill `revisao-testes-documentacao.md`: testes, `.md` do módulo, docstrings, type hints, gate de 80% de cobertura).
