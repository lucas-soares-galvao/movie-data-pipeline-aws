...
- Se a mudança tocar `components.py`/`infrastructure.py` ou lógica Python testável (fora de `app.py`/`forms.py`/`recommendation.py`/`cards.py`), siga o checklist padrão do projeto (skill `revisao-pos-mudanca-codigo`: testes, `.md` do módulo, docstrings, type hints, gate de 100% de cobertura).
