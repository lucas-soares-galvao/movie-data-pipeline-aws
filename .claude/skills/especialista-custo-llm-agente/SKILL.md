...
- **Modelo padrão barato/rápido para uma tarefa estruturada** (`openrouter/deepseek/deepseek-v4.1-flash`, via OpenRouter): mapear texto livre → cláusula WHERE não exige um modelo de fronteira; usar um modelo caro aqui seria custo sem ganho de qualidade perceptível pelo usuário final. `LLM_FALLBACK_MODELS` (padrão `qwen/qwen3.8-flash` — custo/contexto equivalente ao principal, empresa diferente da DeepSeek) só entra em jogo se o principal falhar — não é uma segunda chamada paga por request normal, então não muda esse racional de custo.
