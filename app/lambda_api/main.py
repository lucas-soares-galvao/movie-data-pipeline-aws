...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    only_weekly_table = event.get("only_weekly_table", False)
    skip_weekly = event.get("skip_weekly", False)
    only_monthly_table = event.get("only_monthly_table", False)
    ...
    if only_monthly_table:
        start_year = current_year - 1
        end_year = current_year - 1
        loop_end_year = current_year - 1

    if not only_weekly_table:
        ...
        logger.info("only_weekly_table=True: pulando coleta de genre, configuration e watch_providers_ref.")
    ...
    if content_type == "movie" and table_now_playing and not only_monthly_table:
