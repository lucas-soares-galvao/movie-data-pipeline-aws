"""queries.py — Query SQL de unificação do job Glue AGG."""
...
movie_details_ranked AS (
    SELECT id, runtime, overview_en, overview_pt, poster_path_en, backdrop_path_en,
        ...
        keywords, keywords_pt, certification, theatrical_release_date_br,
...
-- CAST(NULL AS TIPO): o Athena exige que ambos os lados de um UNION ALL tenham o mesmo
-- tipo de dado. NULL sozinho é sem tipo; o CAST informa ao engine o tipo esperado.
details AS (
    ...
    SELECT id, 'tv' AS media_type,
           ...
           actor_names, director, screenplay, music_composer,
           producer, cinematographer, editor,
           keywords, keywords_pt, certification, NULL AS theatrical_release_date_br,
           trailer_url, imdb_id,
           CAST(NULL AS ARRAY<VARCHAR>) AS origin_country,
           recommended_titles, recommended_ids, similar_titles, similar_ids, alternative_titles,
           created_by, networks, in_production, last_air_date, tv_type,
...
-- WITH ORDINALITY gera uma coluna "ord" com o índice (1, 2, 3...) de cada elemento
-- expandido pelo UNNEST — usada no ORDER BY para preservar a ordem original do array.
...
-- para obter nomes em português. Fallback para nome em inglês se não houver tradução.
-- WITH ORDINALITY: mesmo padrão de production_countries_resolved acima.
...
-- COMO funciona o pareamento por índice (ORDINALITY):
--   split(recommended_ids, ', ')     → ['123', '456']   → t_id: (ord=1,'123'), (ord=2,'456')
--   split(recommended_titles, ', ')  → ['Inception','Duna'] → t_en: (ord_en=1,...), (ord_en=2,...)
--   WHERE t_id.ord = t_en.ord_en emparelha pelo índice: ID 123 ↔ 'Inception', 456 ↔ 'Duna'.
--   LEFT JOIN em unified busca o título em pt-BR; COALESCE usa o en como fallback.
...
-- Mesma lógica de resolução para títulos similares (ver recommended_resolved acima).
similar_resolved AS (
    SELECT
        d.id,
        d.media_type,
...
SELECT
    ...
    actor_names, director, screenplay, music_composer,
    producer, cinematographer, editor,
    keywords_pt, certification, theatrical_release_date_br,
    trailer_url, imdb_id,
