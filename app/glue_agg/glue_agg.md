...
Antes da seção do AGG, o step summary também mostra uma seção "Backfill" com o resumo real do que o script fez (extraído do log via `grep`, mesmo formato de log de todos os 7 scripts) e qualquer linha `ERROR` registrada — relevante porque `exit 0` nem sempre significa "toda unidade teve sucesso" (`backfill_enriquecimento.py` é soft-fail-continue, `backfill_data_quality.py` é fire-and-forget; ver `scripts/scripts.md`).
