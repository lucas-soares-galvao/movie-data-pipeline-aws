...
2. Executa uma query SQL complexa no **Athena** que:
   ...
   - Faz `LEFT JOIN` com gêneros, idiomas, países, detalhes (runtime/temporadas, elenco, diretor, roteiristas, compositor, keywords_pt, país de origem, classificação indicativa, trailer, coleção, produtoras, status, tagline, IMDB ID, campos TV), plataformas de streaming e a tabela `now_playing` (para filmes em cartaz nos cinemas)
