...
### Autenticação e administração (`login.py`, `admin.py`, `infrastructure.py` — Cognito User Pool)
...
Quatro telas, alternadas dentro do mesmo `app.py` via `st.session_state["auth_view"]`/`st.session_state["current_view"]` (nunca multipage nativo do Streamlit — evitaria vazar a existência da tela de admin na sidebar/URL para quem ainda não fez login):

- **Login** (`login.py::_render_login_form`) — e-mail + senha, valida via `infrastructure.authenticate()` (`AdminInitiateAuth`). Dois botões-link levam a Cadastro/Esqueci a senha.
- **Novo cadastro** (`_render_signup`) — nome/e-mail/senha/confirmar. Chama `infrastructure.sign_up()` (`SignUp`), que grava a conta em estado **`Unconfirmed`** — o pool é configurado com `auto_verified_attributes = []` de propósito (`infra/lightsail_ia.tf`), então nenhum código de verificação é enviado nesta etapa: quem aprova o acesso é o admin, não uma prova de posse do e-mail (checagem redundante já que o admin revisa cada cadastro manualmente). `infrastructure.notify_new_signup()` publica no tópico SNS `filmbot_new_signup` logo em seguida, para o admin saber que há alguém esperando sem precisar checar o painel periodicamente.
- **Esqueci a senha** (`_render_forgot_password_request`/`_render_forgot_password_confirm`, controlado por `st.session_state["reset_step"]`) — único fluxo com código: e-mail → `infrastructure.request_password_reset()` (`ForgotPassword`, o Cognito gera/envia/expira o código sozinho, pelo remetente nativo dele, sem SES) → código + nova senha → `infrastructure.confirm_password_reset()` (`ConfirmForgotPassword`). Só funciona se o e-mail estiver marcado como verificado — ver nota sobre `approve_signup()` abaixo. O subtítulo da segunda etapa já avisa para checar a pasta de spam (texto da UI do FilmBot, não do e-mail do Cognito — customizar o corpo do e-mail exigiria SES, decisão de escopo já registrada em `infra/lightsail_ia.tf`/`infra/docs/recursos.md`). Botão **Reenviar código** (chama `request_password_reset()` de novo, o que invalida o código anterior) com cooldown de 60s por IP (`_reset_attempt_history`, mesmo padrão `events_in_window`/`seconds_until_available`/`load_countdown_script` do bloqueio de tentativas de login incorretas, linha abaixo) — evita reenvio repetido em sequência; o teto real de segurança continua sendo a quota do próprio Cognito (5–20 requisições de `ForgotPassword`/`ConfirmForgotPassword` por usuário por hora, não configurável).
- **Painel admin** (`admin.py::render_admin_panel`, chamado por `app.py` quando `st.session_state["is_admin"]` — setado no login via `infrastructure.is_admin()`, que checa o grupo Cognito `admins`) — uma única tabela centralizada (`_build_rows()`, combinando `list_pending_users()` + `list_active_users()`) com colunas Nome, E-mail, Admin (sim/não — uma chamada `infrastructure.is_admin()` extra por linha), Status (Novo/Ativo/Revogado) e Ação, renderizada linha a linha via `st.columns` (mesma proporção no cabeçalho e nos dados, pro alinhamento ficar exato — não há `st.dataframe`/`st.table` neste app). A Ação varia por linha: cadastro novo (`Status = Novo`) ganha os dois botões — ✓ **Aprovar** (`infrastructure.approve_signup()`: `AdminConfirmSignUp` **+** `AdminUpdateUserAttributes(email_verified=true)` — o segundo passo é obrigatório e fácil de esquecer, já que `AdminConfirmSignUp` sozinho não verifica o e-mail, e sem isso o "esqueci a senha" desse usuário falha silenciosamente mais tarde) e ✕ **Reprovar** (`infrastructure.reject_signup()`: `AdminDeleteUser`, exclui o cadastro por completo — decisão do projeto, sem histórico de reprovados); usuário existente ativo (`Status = Ativo`) ganha só ✕ **Revogar**; usuário existente já revogado (`Status = Revogado`) ganha só ✓ **Restaurar** (`revoke_access()`/`restore_access()`: `AdminDisableUser`/`AdminEnableUser`, reversível). Os botões usam o parâmetro nativo `icon=` do `st.button` (`:material/check:`/`:material/close:`, sem label) em vez do helper `icon()` de `components.py` (que monta SVG inline via `st.markdown` — não injetável dentro do label de um widget `st.button`).
...
**Fora do escopo desta versão:** notificar o usuário por e-mail quando aprovado/reprovado exigiria configurar SES (identidade verificada + pedido de *production access* — único passo burocrático de toda a proposta, mas sem custo e resolvido em até 24h pela AWS). Sem isso, quem se cadastrou só descobre que foi aprovado tentando logar de novo.

**Rate limit de tentativas de login continua sendo por IP, não por conta** (`_login_attempt_history`, `login.py`) — o Cognito tem bloqueio por tentativa malsucedida nas *Advanced Security Features*, mas são pagas à parte e não foram adotadas; o rate-limit já existente é a única defesa de força bruta.

### Interface (`app.py`, `login.py`, `admin.py`, `recommendation.py`, `cards.py`, `infrastructure.py`)

`app.py` é só o orquestrador: bootstrap (`infrastructure.py`), chama a tela de login (`login.py`, que interrompe a execução com `st.stop()` se o usuário não estiver autenticado), renderiza o cabeçalho (com botão extra "Painel Admin" quando `is_admin`) e chama, em sequência, o formulário de preferência/busca assíncrona (`recommendation.py`) e a exibição dos resultados (`cards.py`) — ou `admin.py::render_admin_panel()` no lugar dos dois, quando `current_view == "admin"`. É puramente organização de código — a experiência do usuário continua sendo uma tela única (login → formulário → cards no mesmo rerun), sem navegação multipage do Streamlit. `recommendation.py` escreve `titles`/`search_error`/`search_completed` em `st.session_state`; `cards.py` só lê essas chaves, sem import entre os dois. `infrastructure.py` reúne o bootstrap de processo (senha via Secrets Manager, logging CloudWatch), as chamadas ao Cognito/SNS da autenticação e os utilitários genéricos de rate limiting (IP do cliente, janela deslizante, segundos restantes) usados tanto por `login.py` quanto por `recommendation.py`, cada um com seu próprio dict de histórico.
...
- **Botão "Entrar" do login desabilitado sem senha, em tempo real:** `disabled=_locked_out or not password` no `st.button(key="btn_entrar")`, com `[data-testid="stButton"] > button:disabled { opacity: 0.5; box-shadow: none }` em `login.css`. Habilita/desabilita a cada tecla digitada no campo de senha — não só no próximo rerun do Streamlit (que só ocorre ao perder o foco/Enter) — via `static/js/login_button_toggle.js` (injetado por `load_login_button_toggle_script(_locked_out)`), mesmo padrão de `contador_caracteres.js`/`window.parent.document` já usado pelo botão "Recomendar"; a flag `locked_out` é passada pro script pro mesmo motivo de `rate_limited` em `contador_caracteres.js` — o JS nunca reabilita um botão que a Python travou por bloqueio de tentativas. Na tela "Esqueci a senha" (`reset_email`), o mesmo script recebe `email_key="reset_email"` e passa a marcar o campo de e-mail com borda verde/vermelha (`.email-valid`/`.email-invalid`, `login.css`) a cada tecla, com base na mesma regex de `_EMAIL_RE` em `login.py` — formato inválido também mantém o botão "Enviar código →" desabilitado. A tela de login não passa `email_key` (default vazio): lá o e-mail só precisa existir para autenticar, formato inválido já falha na autenticação normalmente
- **Senha/confirmar senha/e-mail dinâmicos (cadastro e redefinir senha):** `static/js/password_requirements_gate.js` (injetado por `load_password_requirements_gate_script(password_key, confirm_key, button_key, email_key=...)`) substitui `login_button_toggle.js` nas telas de cadastro e "Redefinir senha" — a cada tecla digitada em qualquer campo do formulário, marca "Confirmar senha" com borda verde/vermelha (`.password-match`/`.password-mismatch`, `login.css`) e mantém o botão de submit desabilitado até todos os campos estarem preenchidos, senha e confirmação coincidirem, e a senha atender à política mínima. Logo abaixo do campo "Confirmar senha", `render_password_requirements()` (`components.py`) renderiza uma lista fixa (`id="password-requirements"`) com um item por critério (8 a 16 caracteres, minúscula, maiúscula, número, símbolo, senha e confirmação iguais) que o mesmo script atualiza para ✓/✗ (`req-met`/`req-unmet`) em tempo real — neutro (`•`) enquanto o respectivo campo estiver vazio, pro usuário não abrir a tela já com tudo em vermelho. Os 5 primeiros critérios (`CRITERIA` no JS, mesma lista de chaves de `data-req` no Python) espelham `_validate_password()` em `login.py`, que continua a fonte de verdade — este mecanismo é só antecipação de UX, a chamada ao Cognito valida de novo no submit. O critério "match" fica fora de `CRITERIA` (reage a senha + confirmação, não só à senha) e reaproveita a mesma comparação já usada pra decorar a borda do campo "Confirmar senha". Só a tela de cadastro passa `email_key="signup_email"` (redefinir senha não tem campo de e-mail digitado — o e-mail já foi confirmado no passo anterior do fluxo): o campo de e-mail ganha a mesma borda verde/vermelha (`.email-valid`/`.email-invalid`, `login.css`), com base na mesma regex de `_EMAIL_RE`, e formato inválido também mantém "Criar cadastro →" desabilitado
...
| `infrastructure.py` | `sign_up(email, password, name)` | Cadastra um usuário no Cognito (`SignUp`) — fica `Unconfirmed` até `approve_signup()` |
| `infrastructure.py` | `authenticate(email, password)` | Valida e-mail/senha (`AdminInitiateAuth`); retorna `"ok"`/`"pending"`/`"invalid"` |
| `infrastructure.py` | `is_admin(email)` | True se o e-mail pertence ao grupo Cognito `admins` (`AdminListGroupsForUser`) |
| `infrastructure.py` | `request_password_reset(email)` / `confirm_password_reset(email, code, new_password)` | `ForgotPassword`/`ConfirmForgotPassword` — código gerado/enviado/expirado pelo próprio Cognito |
| `infrastructure.py` | `list_pending_users()` / `list_active_users()` | `ListUsers` filtrado por status `UNCONFIRMED`/`CONFIRMED`, usado pelo painel admin |
| `infrastructure.py` | `approve_signup(email)` | `AdminConfirmSignUp` + `AdminUpdateUserAttributes(email_verified=true)` — os dois passos são obrigatórios (ver seção "Autenticação e administração") |
| `infrastructure.py` | `reject_signup(email)` | `AdminDeleteUser` — exclui o cadastro pendente por completo |
| `infrastructure.py` | `revoke_access(email)` / `restore_access(email)` | `AdminDisableUser`/`AdminEnableUser` — bloqueia/libera login de um usuário já aprovado |
...
| `infrastructure.py` | `notify_new_signup(email, name)` | Publica no tópico SNS `filmbot_new_signup` (destino fixo — o admin), avisando de um cadastro novo pra revisar |
| `login.py` | `render_login(client_ip)` | Renderiza a view de autenticação ativa (login/cadastro/esqueci senha, `st.session_state["auth_view"]`), com bloqueio por `_MAX_LOGIN_ATTEMPTS` tentativas via `_login_attempt_history`; interrompe a execução com `st.stop()` se não autenticado, ou retorna sem efeito se já autenticado |
| `admin.py` | `render_admin_panel()` | Renderiza o painel admin (cadastros novos + usuários ativos, com ações Aprovar/Reprovar/Revogar/Restaurar); não repete o gate de `is_admin`, confia no chamador (`app.py`) |
...
| `static/css/admin.css` | CSS do painel admin | `.admin-section`/`.admin-user-row` e os botões Aprovar/Reprovar/Revogar/Restaurar (seletor por substring `[class*="st-key-btn_aprovar_"]`, já que a `key` é dinâmica por e-mail) |
...
| `static/js/login_button_toggle.js` | Script de habilitar/desabilitar o botão de submit das 4 telas de autenticação | Observa **todos** os campos `data-testid="stTextInput"` visíveis (só um formulário renderiza por vez) e atualiza o `disabled` do botão indicado por `button_key` (`load_login_button_toggle_script(locked_out, button_key=..., email_key=...)`) a cada tecla digitada em qualquer um deles (exceto quando `lockedOut`); com `email_key` informado (só "esqueci a senha"), também marca o campo de e-mail com borda verde/vermelha por formato e inclui isso no gate |
| `static/js/password_requirements_gate.js` | Script de senha/confirmar senha dinâmicos (cadastro e redefinir senha) | Substitui `login_button_toggle.js` nessas duas telas: marca "Confirmar senha" com borda verde/vermelha, atualiza a lista de critérios (`id="password-requirements"`, `render_password_requirements()`) para ✓/✗ por item, e mantém o botão de submit desabilitado até tudo bater — a cada tecla digitada, via `load_password_requirements_gate_script(password_key, confirm_key, button_key, email_key=...)`; com `email_key` informado (só cadastro), também marca o campo de e-mail com borda verde/vermelha por formato e inclui isso no gate |
