...
- **Título do hero (`st.container(key="hero-section")`):** "O que você quer **assistir** hoje?" (`.hero-heading`, 40px, `font-weight:300`, a palavra "assistir" em laranja via `.accent-gradient-text`) — `text-wrap: balance` evita que "hoje?" quebre sozinho numa 2ª linha (comportamento padrão do navegador dentro dos ~616px úteis de `.st-key-hero-section`, sem essa propriedade); navegadores sem suporte (fora do Chrome 114+/Firefox 121+/Safari 17.5+) simplesmente ignoram a regra e mantêm a quebra padrão de antes, sem quebrar nada. **`margin-top` de `.hero-heading-wrap` é 8px, não 24px:** entre `.st-key-header-row` e este wrap existe um container invisível extra — o `<style>` injetado por `load_recommendation_css()` (`render_recommendation()`, chamado antes do `st.container(key="hero-section")`) — que soma o `gap:16px` nativo do `stVerticalBlock` **duas vezes** (cabeçalho→injetor, injetor→hero-section) em vez de uma, mesmo padrão do injetor `hero-scripts` já documentado acima. `8px + 16px + 16px = 40px`, batendo com o mesmo respiro usado no resto da página (contador/último card→rodapé, rodapé→fim da página, ver bullet "Largura do rodapé" acima) — medido via Playwright (`getBoundingClientRect`), depois de o valor de 24px (assumido só pela leitura do código) ter batido 56px na prática. **`padding:0 !important` no `<h1>`:** o Streamlit aplica um padding próprio (~20px/16px, medido via inspeção real do DOM) em todo `<h1>` renderizado dentro de markdown, com especificidade maior que a classe sozinha — sem o `!important` (tanto no padding quanto no `margin: 0 0 4px` logo abaixo), o respiro entre o heading e "Digite ou grave o seu pedido" (`.hero-subtitle`) ficava ~4x maior que o `margin` declarado sozinho sugeria. Botão "Recomendar" sem emoji no rótulo (só o texto, igual ao estado desabilitado que já não tinha)
