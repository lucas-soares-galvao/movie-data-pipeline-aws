...
- **Largura do rodapé (`render_footer()`):** antes de pesquisar, o rodapé fica com a mesma largura do hero (640px, centralizado, `.footer { max-width: 640px; margin: auto }`). `.grid-titles` (resultados) não tem largura própria — ocupa a largura natural do `block-container`. Quando há resultado na tela, `body:has(.grid-titles) .footer { max-width: none }` destrava o rodapé pra acompanhar essa largura maior, em vez de ficar preso nos 640px do hero. **`margin-top` negativo (`-36px`):** o `.stVerticalBlock` principal do Streamlit aplica `gap:16px` entre TODOS os elementos de topo da página, incluindo containers de altura zero que não renderizam nada visualmente — hoje só o injetor de JS do formulário (`hero-scripts`, sempre presente, `display:none`). Erro/sem-resultado de busca (`render_feedback()`, chamado em `recommendation.py` dentro de `hero-actions`, junto do rate limit — ver abaixo) não entram nessa conta: a caixa cresce o `hero-actions` existente em vez de adicionar um item novo ao `stVerticalBlock`, então o gap escondido antes do rodapé é o mesmo com ou sem essas mensagens visíveis (confirmado via Playwright, `getBoundingClientRect()`, gap idêntico ~16px nos estados vazio/só erro/rate-limit+erro juntos). O `margin-top` negativo compensa esse gap escondido e iguala o respiro visível ao gap nativo "Recomendar"→"Consultas restantes" (16px) — calibrado por medição (Playwright), não por soma das margens declaradas, já que a resposta do gap visível a mudanças nesse valor não é 1:1 (colapso de margem com o gap escondido)
