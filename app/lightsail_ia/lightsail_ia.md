...
Além de digitar, o usuário pode gravar a preferência em áudio pelo widget nativo `st.audio_input`. Ao parar a gravação, o app **não transcreve automaticamente** — mostra uma etapa de confirmação ("▶️ Usar gravação" / "✕ Cancelar") para o usuário decidir se aquele take deve ser enviado para transcrição ou descartado. Só ao clicar em "Usar gravação" o áudio é transcrito por `transcribe_preference()` (`agent.py`) usando Whisper via `litellm` — modelo configurável por `TRANSCRIPTION_MODEL` (padrão: Groq Whisper Large v3 Turbo, rápido e barato), com `language="pt"` fixo. O texto resultante pré-popula o `st.text_area` de preferência, permanecendo totalmente editável antes de clicar em "Recomendar".

- **Confirmação antes de transcrever:** o `st.audio_input` nativo não expõe nenhuma forma de cancelar uma gravação **em andamento** (o único botão durante o estado `recording` é "parar", que sempre finaliza e envia o áudio para o backend) — essa limitação foi confirmada inspecionando o bundle JS interno do widget. Por isso a etapa de "cancelar" acontece **depois** que a gravação já parou, mas **antes** da chamada à API de transcrição: `audio_awaiting_confirmation` guarda esse estado intermediário e `audio_pending_bytes` os bytes do áudio ainda não confirmado. Ao clicar "✕ Cancelar", `audio_widget_seq` é incrementado para forçar uma nova instância do `st.audio_input` (troca de `key`) — o único jeito de "esvaziar" visualmente um áudio já gravado, já que o widget não tem API para isso.
- **Ícone de descarte durante a gravação:** `static/audio_cancel_recording.js` (injetado por `load_audio_cancel_script()`, mesmo padrão de `contador_caracteres.js`) mostra um ícone 🗑 ao lado do botão nativo enquanto o estado é `recording`. Como a função interna `cancel()` do gravador não é acessível fora do componente React, o ícone simula "parar" (clica no botão nativo) e arma uma flag em `localStorage`; assim que o botão "✕ Cancelar" da confirmação aparece, o script clica nele automaticamente, descartando o áudio sem nunca chamar `transcribe_preference`. Efeito colateral aceito: a UI de confirmação aparece por um instante (flash) antes do auto-descarte, já que "parar" sempre envia o áudio antes de qualquer descarte ser possível.
...
- Confirmação "▶️ Usar gravação" / "✕ Cancelar" após parar a gravação de áudio, antes de transcrever (ver seção "Entrada alternativa" acima) — o botão "Usar gravação" fica desabilitado quando o rate limit de transcrições é atingido
