...
- **Tema claro/escuro dinâmico** (`theme.css`, tokens `:root`/`var()` consumidos pelos demais arquivos CSS)
  ...
  **Décima sexta rodada — gap entre o grid de colunas e a borda inferior, causa raiz era a
  ...
  `.admin-table-wrap` mede ~549.6px de largura, bem menor que o `min-width:1180px` da `<table>` —
  como o wrapper tem `overflow-x:auto` e o conteúdo é mais largo que o container, o próprio
  navegador reserva uma faixa pra a scrollbar horizontal na parte de baixo do elemento (a mesma
  suspeita já registrada, sem confirmação, na rodada 15). Sem nenhum estilo, essa faixa reservada
  é fundo liso sem contraste nenhum no tema escuro — lê como gap desconectado, não como scrollbar.
  Decisão do usuário: manter as duas scrollbars (horizontal e vertical) visíveis e funcionais, em
  vez de escondê-las — não dava pra esticar o `border-right` das colunas por dentro dessa faixa
  com precisão (ela não pertence à `<table>` nem se desloca junto quando a tabela rola
  horizontalmente), então a correção estiliza a própria scrollbar nativa do wrapper
  (`::-webkit-scrollbar-track`/`-thumb` no Chrome/Edge/Safari, `scrollbar-width`/`scrollbar-color`
  no Firefox) com os mesmos tons já usados na tabela — trilho em `var(--overlay-08)` (mesma
  família da borda de card, `base.css`), polegar em `var(--overlay-16)` (mesmo tom das linhas de
  grade/`border-right`/`border-bottom` da própria tabela). A faixa passa a ler como um divisor
  desenhado de propósito, encostado na borda, em vez de vão vazio acidental. Espessura fixada em
  8px (`::-webkit-scrollbar { width/height: 8px }`), o que também tornou o tamanho da faixa
  previsível em vez de depender do padrão variável do SO/navegador — `max-height` de
...
- Logging de erros: exceções na busca (e na transcrição de áudio) são registradas via `logging.exception()` e enviadas ao CloudWatch Logs (quando `CLOUDWATCH_LOG_GROUP` está configurada) para diagnóstico em produção. A mensagem inclui `user_email`/`user_name` (lidos de `st.session_state` no momento do erro) diretamente no texto — não via `extra`/atributos do `LogRecord`, já que o `CloudWatchLogFormatter` do `watchtower` só injeta atributos extras na mensagem quando `add_log_record_attrs` está configurado no handler, o que não é o caso aqui — permite identificar/contatar o usuário afetado por aquele erro específico
- Alerta agregado por e-mail (infraestrutura, não app): `infra/cloudwatch_logs.tf` (`aws_cloudwatch_log_metric_filter.filmbot_error_filter`) conta ocorrências das mesmas duas mensagens de erro no log group `lightsail_filmbot`; `infra/cloudwatch_alarms.tf` (`aws_cloudwatch_metric_alarm.filmbot_error_alarm`) dispara já na primeira ocorrência, mas só executa a notificação na transição OK→ALARM (não a cada erro) — vira "um alerta por incidente", não um e-mail por usuário/ocorrência. A notificação em si segue o mesmo padrão já usado para Lambda/EventBridge: um `aws_cloudwatch_event_rule` captura a mudança de estado do alarme e publica no tópico SNS (`infra/sns_topics.tf`, `filmbot_error_notifications`) via `input_transformer`, com a política do tópico (`infra/iam_policies.tf`) restrita a esse rule ARN específico
...
| `static/css/admin.css` | CSS do painel admin | `.admin-empty`, `.st-key-admin-table` (`width:100%` + `overflow-x:auto` pra manter a mesma caixa de 640px dos cards de perfil/senha, com `min-width` forçado no `stHorizontalBlock` de cabeçalho/linha pra habilitar scroll horizontal local quando os dados não cabem — tabela de usuários em DOM real via `st.columns()`/`st.button()`, não mais `st.dataframe`, ver "Nona rodada" da seção "Autenticação e administração"), `.admin-header-cell`/`.admin-cell`/`.admin-cell-center` (célula de cabeçalho/dado, tema via `theme.css`), o pill verde/vermelho dos botões Aprovar/Revogar (`--feedback-success-*`/`--feedback-error-*`, centralizado na coluna via `width:100%` em `stElementContainer`+`stButton`) e a grade completa (`border-right` por `stColumn`, `border-bottom` do cabeçalho e moldura externa da caixa, todos via `var(--overlay-*)` — ver "Décima rodada" da seção "Autenticação e administração"); `[data-testid="stDialog"] > div` (largura do card visível do modal de confirmação Reprovar/Revogar restrita a 440px — `st.dialog(width="small")` sozinho ainda permite até 500px; o próprio `[data-testid="stDialog"]` é o overlay de tela cheia que centraliza esse filho via flexbox, não o card — aplicar a largura nele quebraria a centralização), `flex-direction:row!important`/`min-width:0` nos botões Cancelar/Confirmar (mesma técnica de `.st-key-admin-nav` em `profile.css`, pra impedir que o Streamlit empilhe os dois em telas estreitas), e `width:100%` em `[data-testid^="stBaseButton"]` (o `width="stretch"` do Python, sozinho, deixa cada botão com ~140px fixos alinhado à esquerda da própria coluna de ~188px em vez de preenchê-la — confirmado via Playwright/getComputedStyle — o que quebrava a simetria esquerda/direita do card) |
