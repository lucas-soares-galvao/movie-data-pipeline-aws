...
- **Senha/confirmar senha dinâmicos (cadastro e redefinir senha):** `static/js/password_requirements_gate.js` (injetado por `load_password_requirements_gate_script(password_key, confirm_key, button_key)`) substitui `login_button_toggle.js` nas telas de cadastro e "Redefinir senha" — a cada tecla digitada em qualquer campo do formulário, marca "Confirmar senha" com borda verde/vermelha (`.password-match`/`.password-mismatch`, `login.css`) e mantém o botão de submit desabilitado até todos os campos estarem preenchidos, senha e confirmação coincidirem, e a senha atender à política mínima. Logo abaixo do campo "Confirmar senha", `render_password_requirements()` (`components.py`) renderiza uma lista fixa (`id="password-requirements"`) com um item por critério (8 a 16 caracteres, minúscula, maiúscula, número, símbolo, senha e confirmação iguais) que o mesmo script atualiza para ✓/✗ (`req-met`/`req-unmet`) em tempo real — neutro (`•`) enquanto o respectivo campo estiver vazio, pro usuário não abrir a tela já com tudo em vermelho. Os 5 primeiros critérios (`CRITERIA` no JS, mesma lista de chaves de `data-req` no Python) espelham `_validate_password()` em `login.py`, que continua a fonte de verdade — este mecanismo é só antecipação de UX, a chamada ao Cognito valida de novo no submit. O critério "match" fica fora de `CRITERIA` (reage a senha + confirmação, não só à senha) e reaproveita a mesma comparação já usada pra decorar a borda do campo "Confirmar senha"
...
| `static/js/login_button_toggle.js` | Script de habilitar/desabilitar o botão de submit das 4 telas de autenticação | Observa **todos** os campos `data-testid="stTextInput"` visíveis (só um formulário renderiza por vez) e atualiza o `disabled` do botão indicado por `button_key` (`load_login_button_toggle_script(locked_out, button_key=...)`) a cada tecla digitada em qualquer um deles (exceto quando `lockedOut`) |
| `static/js/password_requirements_gate.js` | Script de senha/confirmar senha dinâmicos (cadastro e redefinir senha) | Substitui `login_button_toggle.js` nessas duas telas: marca "Confirmar senha" com borda verde/vermelha, atualiza a lista de critérios (`id="password-requirements"`, `render_password_requirements()`) para ✓/✗ por item, e mantém o botão de submit desabilitado até tudo bater — a cada tecla digitada, via `load_password_requirements_gate_script(password_key, confirm_key, button_key)` |
