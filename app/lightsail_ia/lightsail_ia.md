...
- **Painel admin** (`admin.py::render_admin_panel`, chamado por `app.py` quando `st.session_state["is_admin"]` — setado no login via `infrastructure.is_admin()`, que checa o grupo Cognito `admins`) — lista cadastros novos (`Unconfirmed`) com botões **Aprovar** (`infrastructure.approve_signup()`: `AdminConfirmSignUp` **+** `AdminUpdateUserAttributes(email_verified=true)` — o segundo passo é obrigatório e fácil de esquecer, já que `AdminConfirmSignUp` sozinho não verifica o e-mail, e sem isso o "esqueci a senha" desse usuário falha silenciosamente mais tarde) e **Reprovar** (`infrastructure.reject_signup()`: `AdminDeleteUser`, exclui o cadastro por completo — decisão do projeto, sem histórico de reprovados). Lista também usuários já aprovados, com **Revogar**/**Restaurar** acesso (`revoke_access()`/`restore_access()`: `AdminDisableUser`/`AdminEnableUser`, reversível).
