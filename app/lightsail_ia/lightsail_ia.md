...
- **Limite de duração:** áudios com mais de 15 segundos (`_MAX_AUDIO_SECONDS`) são rejeitados **antes** de chamar a API de transcrição — a duração é calculada com o módulo padrão `wave` (sem dependência nova), já que `st.audio_input` sempre entrega WAV. O limite é exibido para o usuário como um badge "decorrido / máximo" (ex: `00:07 / 00:15`) ao lado do label do gravador, além do aviso "⚠️ Áudio muito longo" exibido caso a gravação exceda o limite. O badge (`<span id="audio-timer-badge">`, renderizado por `app.py` já com o valor inicial `00:00 / MM:SS`) é atualizado em tempo real por `static/audio_timer.js` (injetado por `load_audio_timer_script()`, mesmo padrão de `contador_caracteres.js`/`window.parent.document`) enquanto o estado é `recording`, lendo o tempo decorrido do timer nativo do widget pelo hook não documentado `data-testid="stAudioInputWaveformTimeCode"` — mesma ressalva de fragilidade a upgrades de versão do Streamlit do contador de caracteres (ver abaixo). O timer nativo do widget continua sendo lido pelo script, mas fica escondido via CSS (`display: none` em `principal.css`) para não duplicar visualmente o mesmo tempo já exibido pelo badge.
...
| `componentes.py` | `load_login_css()`, `load_main_css()`, `load_preference_counter_script()`, `load_audio_cancel_script()`, `load_audio_timer_script()`, `load_textarea_autogrow_script()`, `render_card()`, `render_grid()`, `render_footer()`, `render_login_footer()` | Helpers de renderização HTML com escape contra XSS |
