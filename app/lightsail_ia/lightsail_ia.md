...
- **Largura do rodapé (`render_footer()`):** antes de pesquisar, o rodapé fica com a mesma largura do hero (640px, centralizado, `.footer { max-width: 640px; margin: auto }`). `.grid-titles` (resultados) não tem largura própria — ocupa a largura natural do `block-container`. Quando há resultado na tela, `body:has(.grid-titles) .footer { max-width: none }` destrava o rodapé pra acompanhar essa largura maior, em vez de ficar preso nos 640px do hero. **Respiro acima do rodapé:** padding-top de 16px (estado sem resultado) ou 24px (`body:has(.grid-titles) .footer`, mais `margin-top:16px`) soma 40px de respiro em relação ao conteúdo anterior, igual ao padrão cabeçalho→hero usado no resto da página — ver bullet "Sem linha divisória entre blocos" acima. **Padding vertical simétrico (16px/16px sem resultado, 24px/24px com grid):** o texto fica centralizado dentro da própria caixa do rodapé, em vez do padding-bottom fixo em 8px que existia antes (ajuste a pedido do usuário, comparando com a caixa desenhada visualmente enxuta demais embaixo). **Centralização vertical do rodapé no vão da tela:** quando o conteúdo da página (cabeçalho + busca + grid) é mais curto que a altura da janela, o rodapé passa a flutuar centralizado no espaço vazio entre o fim do conteúdo real e o fim da tela, em vez de ficar colado logo abaixo do grid com um vão desbalanceado sobrando só por baixo dele. Mecanismo: `[data-testid="stMain"] .block-container` ganha `display:flex; flex-direction:column; min-height:100vh/100dvh` (mesma técnica de `login.css`, mas sem `justify-content:center` — aqui só o rodapé deve se mover, cabeçalho/busca/grid continuam colados no topo); o último filho direto do `stVerticalBlock` (`*:last-child`, sempre o wrapper do rodapé — `render_footer()` é a última chamada de `app.py`) recebe `margin-top:auto; margin-bottom:auto`, técnica clássica de flexbox que distribui o espaço livre igualmente acima/abaixo desse item, sem afetar o espaçamento entre os elementos anteriores. Quando o conteúdo já é mais alto que a tela (resultado longo, tela pequena), não sobra espaço livre e o comportamento volta a ser o de antes (rodapé colado ao conteúdo, sem vão). Confirmado via Playwright nos três estados (hero curto, grid curto, conteúdo maior que o viewport) — `*:last-child` em vez de mirar o `data-testid` do wrapper interno do Streamlit (`stElementContainer`) porque esses wrappers não têm testid estável entre versões, mesma cautela já registrada para `.block-container > [data-testid="stVerticalBlock"]` na tela de login
