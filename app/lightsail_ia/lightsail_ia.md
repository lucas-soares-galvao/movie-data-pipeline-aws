...
- **Tela de login (`login-card`, `static/css/login.css`):** mesmo ícone "clapperboard" e mesmo estilo de wrapper do cabeçalho principal — `.login-icon-badge` sem fundo/borda, do mesmo tamanho do ícone (18x18px, mesma lógica de `.header-icon-badge` acima) com o ícone laranja (`.login-icon-badge .icon { color:#f97316 }`, que vence a `.icon` branca de `base.css` por especificidade de seletor descendente) dentro de `.login-brand` (`display:flex; gap:4px`, mesmo princípio de `.header-brand`), ao lado do título "FilmBot" — branco (`.login-title`, sem `.accent-gradient-text`, removida do login: reservada só pra "assistir" no hero). Subtítulo (`.login-subtitle`, `margin-bottom:4px`) logo abaixo, sem divisor (removido por pedido do usuário — `.login-divider`/`<hr>` não existe mais) — o campo de senha vem direto em seguida. **Créditos do rodapé dentro do card:** `render_login_footer()` é chamado dentro do mesmo `with st.container(key="login-card")` do formulário (não mais depois, fora do card) — os créditos (`.footer-login`) renderizam como último elemento dentro da borda arredondada do card, não soltos embaixo dele
