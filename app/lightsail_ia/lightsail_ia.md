...
**Logging de `tool_calls` vazio (LLM não chamou a tool):** também loga `logger.warning` (com a preferência do usuário, `finish_reason` e `model_used`) antes de retornar lista vazia. Esse caso é ambíguo — pode ser falha real do modelo (não seguiu a instrução do `_SYSTEM_PROMPT` de sempre chamar a tool) ou o pedido do usuário genuinamente fora do escopo de filme/série — mas é logado mesmo assim, para diagnosticar com dado real depois qual dos dois predomina. Diferente disso, um resultado vazio do Passo 2 (Athena não encontrou título com os filtros do LLM) **não** é logado — é "não encontrou" legítimo, não falha de busca.
...
**Observabilidade da degradação (três caminhos silenciosos, todos logados via `logger.warning`):** `content` vazio, `content` que não é JSON válido (`json.JSONDecodeError` — inclui o caso de resposta truncada por `max_tokens`, `finish_reason="length"`), e JSON válido mas com `reason` faltando para algum título (`id` ausente ou sem correspondência) — os três casos logam `content` bruto (truncado a 500 chars), `finish_reason` e `model_used`, para diagnosticar sem precisar reproduzir o problema às cegas. Esses campos só chegam de fato ao CloudWatch por causa de `_ExtraFieldsFormatter` (`infrastructure.py::setup_cloudwatch_logging()`) — sem ele, o formatter padrão do Python só emite `%(message)s` e descarta qualquer campo passado via `extra=`, o que também silenciava (antes desse fix) os logs já existentes de token usage/latência por passo/cache hit.
...
- `num_retries=3` — configurado deliberadamente após um problema real de indisponibilidade/erro transitório com o provedor de LLM. Não é pra remover: tirar isso pra ganhar latência trocaria "às vezes demora mais" por "às vezes quebra de novo".
- `LLM_MODEL` fala com o LLM via **OpenRouter** (agregador de provedores) em vez de uma API direta de provedor único — padrão `openrouter/deepseek/deepseek-v4.1-flash`. Além de `num_retries` (que reforça a requisição inteira contra falha transitória de rede), `LLM_FALLBACK_MODELS` (padrão: `qwen/qwen3.8-flash`) usa o recurso nativo de fallback do OpenRouter (`extra_body={"models": [...]}` no `litellm.completion()`): se o modelo principal falhar por qualquer motivo (indisponibilidade, rate limit, erro de validação de contexto), o OpenRouter troca de modelo — inclusive de empresa diferente — **dentro da mesma requisição**, usando a mesma `OPENROUTER_API_KEY`, sem chave adicional. Os dois mecanismos são complementares, não redundantes: um protege a viagem de rede, o outro protege contra o modelo principal estar fora do ar. Só tem efeito quando `LLM_MODEL` usa o prefixo `openrouter/`.
- `extra_body={"reasoning": {"enabled": False}}` desabilita reasoning/thinking tokens (parâmetro nativo do OpenRouter — [docs](https://openrouter.ai/docs/use-cases/reasoning-tokens)) nas duas chamadas. Reasoning tokens contam como tokens de output/cobrança mesmo sem aparecer em `content` — puro desperdício de budget numa tarefa de saída estruturada/curta, sem necessidade de raciocínio em cadeia. Adicionado depois de um incidente real em produção: o `deepseek-v4.1-flash` (modelo de raciocínio) consumiu a maior parte de `_LLM_MAX_TOKENS_STEP3` em reasoning invisível, cortando o JSON de saída no meio (`finish_reason="length"`) e derrubando os motivos de recomendação para `""` em todos os títulos (ver `_log_token_usage`/logs do Passo 3 abaixo).
...
- `max_tokens` explícito — `_LLM_MAX_TOKENS_STEP1 = 300` (saída pequena, só `where_clause`+`limit` via tool call) e `_LLM_MAX_TOKENS_STEP3 = 3000` (motivos de até 15 títulos; elevado de 1500 após o incidente de reasoning tokens acima — mantido como defesa adicional mesmo com reasoning desabilitado). Generoso o bastante pra nunca truncar uma resposta normal — só dá um teto à latência de cauda de uma resposta anormalmente verbosa.
