...
_SYSTEM_PROMPT = (
    ...
    "- language_name (string): nome do idioma em português (ex: 'Inglês', 'Coreano', 'Japonês'). Para filtrar idioma prefira original_language.\n"
    ...
    "- origin_country_name (string): nome do país de origem em português (ex: 'Brasil', 'Estados Unidos', 'Coreia do Sul'). Prefira origin_country; se usar o nome, use lower() + LIKE.\n"
    ...
    "- title_status (string): estado do título, em português. Filmes: 'Lançado', 'Pós-Produção', 'Em Produção', 'Planejado', 'Cancelado', 'Rumor'. Séries: 'Em Exibição', 'Encerrada', 'Cancelado', 'Em Produção', 'Planejado', 'Piloto'\n"
    "- collection_name (string): saga/franquia de filmes, em português quando o TMDB tem tradução (ex: 'Harry Potter - Coleção'). Apenas filmes. Use lower() + LIKE só com o nome da franquia (ex: '%harry potter%').\n"
    ...
    "- production_countries (string): países de produção em português (ex: 'Estados Unidos, Nova Zelândia'). Diferente de origin_country (cultural). Use lower() + LIKE para coproduções.\n"
    "- spoken_languages (string): idiomas falados no título, em português (ex: 'Inglês, Francês'). Use lower() + LIKE.\n"
    ...
    "- recommended_titles (string): títulos recomendados pelo TMDB, em português ou no título original (ex: 'Interstellar, The Prestige'). Use lower() + LIKE para encontrar títulos relacionados.\n"
    ...
    "- Valores textuais de status, país e idioma na tabela estão em português (ex: title_status = 'Lançado'), nunca em inglês.\n"
    ...
    "theatrical_release_date_br futuro, air_date futuro ou title_status = 'Pós-Produção') "
