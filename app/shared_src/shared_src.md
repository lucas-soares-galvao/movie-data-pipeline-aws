...
| `resolve_pt_translation(df, source_column, target_column, idioma_en_column, idioma_pt_column, tentativas_column, detect_fn, translate_fn, max_workers=10, max_tentativas=3)` | Sincroniza `target_column` (já inicializada pelo chamador — nativo do TMDB, cache reaproveitado ou vazia) com `source_column`: detecta `idioma_en_column`/`idioma_pt_column` (só onde ainda vazios — não recalcula o que já foi detectado em execuções anteriores), copia a fonte direto quando ela já é `"pt"` (sem chamar tradutor), traduz as linhas elegíveis (fonte preenchida, `idioma_pt_column != "pt"`, `tentativas_column < max_tentativas`) via `translate_in_parallel`, incrementa `tentativas_column` e redetecta `idioma_pt_column` só nas linhas recém-traduzidas. Basear a elegibilidade no idioma real do **resultado** (em vez de comparar string com a fonte, heurística antiga) evita tanto retraduzir o que já está correto quanto deixar uma mistradução silenciosa (resultado diferente da fonte, mas em outro idioma) marcada como concluída para sempre; `tentativas_column` evita retry infinito de conteúdo genuinamente não traduzível (nomes próprios, termos curtos). Devolve `(df, quantidade traduzida com sucesso)`. Usada por `glue_details`, `scripts/backfill_traducao.py` e `glue_etl` (`name_pt` de países/idiomas) |
