...
| Função | Responsabilidade |
|---|---|
| `translate_text(text, context)` | Traduz texto para português via Google Translate com detecção automática do idioma de origem (`source="auto"`), com backoff entre tentativas (mesmo padrão de `api_get`); uma exceção usa o orçamento completo de 5 tentativas (tende a ser transitório — rede, rate limit), enquanto um resultado idêntico ao original sem exceção desiste em só 2 (normalmente indica que não há o que traduzir — nome próprio, termo emprestado — não bloqueio); uma **página de erro do Google** devolvida no lugar da tradução (`Error 500 (Server Error)!!1...`, ver `is_google_error_page`) também conta como erro com o orçamento completo de 5 tentativas — antes era gravada como se fosse a tradução — e, ao esgotar, loga um `WARNING` (único desfecho de falha fora de `DEBUG`, por indicar bloqueio real do Google); retorna o original se as tentativas se esgotarem, para não interromper o job |
| `is_google_error_page(text)` | `True` se `text` é a página de erro do Google Translate (string que começa com `Error <status> (<motivo>)!!<n>`, regex ancorada — não casa tradução que apenas mencione "Error"). Aceita qualquer tipo (`None`/NaN de colunas de DataFrame devolvem `False`). Usada por `translate_text` (rejeitar o resultado) e por `resolve_pt_translation` (descartar o que já foi gravado) |
...
| Função | Responsabilidade |
|---|---|
| `resolve_translate_fn(provider, translate_google=translate_text, translate_aws=translate_text_aws, aws_fallback_max_chars=2_000_000, get_chars_used_this_month=get_translate_chars_used_this_month)` | Resolve `"google"`/`"aws"` para uma função **composta primário+fallback**: o provider escolhido é tentado primeiro; se falhar (resultado igual ao texto original), o outro serviço é tentado automaticamente. `"google"` (default em todo o pipeline — `glue_details`/`glue_etl` via EventBridge e os backfills manuais) usa AWS Translate como fallback, capado pelo que sobrar do orçamento **mensal** `aws_fallback_max_chars` (default 2.000.000 == free tier mensal do AWS Translate) menos `get_chars_used_this_month()` — consultado ao vivo a cada chamada, não mais um teto fixo resetado por execução (era 6.000/execução antes desta revisão; provou ser pequeno demais pra resgatar o volume real de falhas do Google). `"aws"` (escolha explícita, ex.: testar tradução real da AWS num período curto) usa Google como fallback, sem limite (grátis). `translate_google`/`translate_aws`/`get_chars_used_this_month` são parâmetros (mesmo motivo de `translate_in_parallel`) para que o chamador passe sua própria referência local — a mesma que seus testes fazem mock. Levanta `ValueError` para qualquer outro valor de `provider` |
| `get_translate_chars_used_this_month(cloudwatch_client=None, region="us-east-1")` | Soma o `CharacterCount` que o próprio AWS Translate publica no CloudWatch (namespace `AWS/Translate`, dimensões `LanguagePair`+`Operation="TranslateText"`) desde o dia 1 do mês corrente (UTC) — a mesma métrica que a AWS usa para faturar, consultada ao vivo em vez de um contador próprio persistido (o bucket TEMP, onde vive o checkpoint de backfill, apaga tudo em 1 dia via lifecycle sem filtro de prefixo — não sobreviveria até o fim do mês). Enumera os pares de idioma publicados (`list_metrics`) e soma `Sum` de cada um (`get_metric_statistics`). Se a consulta ao CloudWatch falhar, loga e devolve `sys.maxsize` — trata o orçamento como esgotado em vez de assumir consumo zero, mais seguro financeiramente |
