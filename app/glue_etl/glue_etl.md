...
2. Lê o JSON do S3 SOR para o ano especificado (`read_from_sor`) — o `overview` já vem em pt-BR nativo do TMDB (buscado pelo `lambda_api` com `language=pt-BR`), sem nenhuma etapa de tradução aqui. `read_from_sor` adiciona `overview_idioma_detectado` (via `langdetect`, com fallback AWS Comprehend — ver `shared_utils.idioma`) e `overview_traduzido_pt_br` (booleano puramente derivado, `overview_idioma_detectado == "pt"`, sem nenhuma chamada de tradução) — sinais **puros de diagnóstico**: confirmam se o TMDB de fato devolveu a sinopse em português, ou se caiu silenciosamente para outro idioma quando não tinha tradução. O `glue_agg` usa `overview_idioma_detectado` para decidir se confia no `overview` do discover ou cai para `overview_pt`/`overview_en` do `glue_details`
...
Para `configuration` de TV (países): após ler o JSON, `_add_translation` traduz `english_name` para português via `resolve_translate_fn(TRANSLATE_PROVIDER)` — `"google"` (default deste job) ou `"aws"` — e grava como coluna `name_pt` na SOT (~250 países), delegando a `shared_utils.traducao.resolve_pt_translation` a detecção de idioma da fonte (`name_idioma_detectado_en`) e do resultado (`name_idioma_detectado_pt`), a cópia direta quando `english_name` já é detectado como `"pt"` (caso raro, já que é sempre nome próprio em inglês — mesma otimização do `glue_details` contra retradução infinita) e o teto de tentativas (`name_tentativas_traducao`). O serviço não escolhido é usado automaticamente como fallback caso o primário falhe.
Para `configuration` de Movie (idiomas): mesma abordagem — traduz `english_name` dos idiomas para português pelo mesmo serviço e grava como coluna `name_pt` na SOT (~190 idiomas), com as mesmas colunas `name_idioma_detectado_en`/`name_idioma_detectado_pt`/`name_tentativas_traducao`.
...
| `read_from_sor(bucket, media_type, table_type, year=None, translate_fn=None, s3_bucket_sot=None, table_name=None, detect_fn=None)` | Lê JSON/Parquet da camada SOR; para `discover` adiciona `overview_idioma_detectado` (diagnóstico, via `detect_fn`) e `overview_traduzido_pt_br` (derivado, sem tradução); para `configuration` adiciona tradução `name_pt` (countries em tv, languages em movie) via `translate_fn`, mais `name_idioma_detectado_en`/`name_idioma_detectado_pt`/`name_tentativas_traducao` via `detect_fn`. Quando `s3_bucket_sot`/`table_name` são informados (só relevante para `configuration`), lê a tabela já gravada na SOT via `read_existing_configuration` e usa como cache de tradução (`_add_name_pt_countries`/`_add_name_pt_languages`) |
| `_add_translation(df, description, key_column, translate_fn=None, previous_df=None, detect_fn=None)` | Reaproveita `name_pt` de `previous_df` via `reuse_existing_translation` (`shared_utils.traducao`) quando `english_name` não mudou para a mesma `key_column`, antes de traduzir sequencialmente (`max_workers=1` — volumes pequenos, ~250 itens). O resto do fluxo (detecção de idioma da fonte e do resultado, cópia direta, tradução via `translate_fn`, teto de tentativas) é responsabilidade de `shared_utils.traducao.resolve_pt_translation` — grava `name_idioma_detectado_en`, `name_idioma_detectado_pt`, `name_pt` e `name_tentativas_traducao` |
...
| `resolve_pt_translation(df, source_column, target_column, idioma_en_column, idioma_pt_column, tentativas_column, detect_fn, translate_fn, max_workers, max_tentativas)` | `shared_utils.traducao` | Detecta o idioma da fonte e do resultado (só onde ainda vazios), copia a fonte direto quando ela já é `"pt"`, traduz as linhas elegíveis e incrementa o contador de tentativas. Usada por `_add_translation` (`name_pt`); compartilhada com `glue_details` e `scripts/backfill_traducao.py` |
...
| `add_detected_language_column(df, source_column, target_column, detect_fn=None, only_missing=False)` | `shared_utils.idioma` | Aplica `detect_fn` a cada valor de `source_column`, gravando o idioma detectado em `target_column` — usada diretamente para `overview_idioma_detectado` (discover; a partição é sempre reconstruída do zero, sem `only_missing`) |
...
- **langdetect** — detecção local de idioma para `overview_idioma_detectado` (discover) e `name_idioma_detectado_en`/`name_idioma_detectado_pt` (configuration)
