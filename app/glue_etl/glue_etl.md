...
2. Lê o JSON do S3 SOR para o ano especificado (`read_from_sor`) — o `overview` já vem em pt-BR nativo do TMDB (buscado pelo `lambda_api` com `language=pt-BR`), sem nenhuma etapa de tradução aqui. `read_from_sor` adiciona `overview_detected_language` (via `langdetect`, com fallback AWS Comprehend — ver `shared_utils.idioma`) e `overview_translated_pt_br` (booleano puramente derivado, `overview_detected_language == "pt"`, sem nenhuma chamada de tradução) — sinais **puros de diagnóstico**: confirmam se o TMDB de fato devolveu a sinopse em português, ou se caiu silenciosamente para outro idioma quando não tinha tradução. O `glue_agg` usa `overview_detected_language` para decidir se confia no `overview` do discover ou cai para `overview_pt`/`overview_en` do `glue_details`
...
Para `configuration` de TV (países): após ler o JSON, `_add_translation` traduz `english_name` para português via `resolve_translate_fn(TRANSLATE_PROVIDER)` — `"aws"` (primário no deploy atual, `var.translate_provider`) ou `"google"` (default do *código*) — e grava como coluna `name_pt` na SOT (~250 países), delegando a `shared_utils.traducao.resolve_pt_translation` a detecção de idioma da fonte (`name_detected_language_en`) e do resultado (`name_detected_language_pt`), a cópia direta quando `english_name` já é detectado como `"pt"` (caso raro, já que é sempre nome próprio em inglês — mesma otimização do `glue_details` contra retradução infinita) e o teto de tentativas (`name_translation_attempts`). O serviço não escolhido é usado automaticamente como fallback caso o primário falhe.
...
Em ambos os casos (países e idiomas), uma execução do job também repara `name_pt` já gravado com a página de erro do Google (`Error 500 (Server Error)!!1...`, gravada como "tradução" por versões antigas de `translate_text`): o passo 0 de `resolve_pt_translation` (ver `shared_utils/traducao.py`) descarta esse valor e zera o contador de tentativas da linha, que volta a ser retraduzida na mesma execução (Google, com fallback AWS).
...
| | Descrição |
|---|---|
| **Entrada** | Argumentos do Glue job: `MEDIA_TYPE`, `TABLE_TYPE`, `TABLE_NAME`, `DATABASE`, `YEAR` (apenas discover), `END_YEAR`, nomes dos buckets e jobs, `TRANSLATE_PROVIDER` (opcional; default do *código* é `"google"`, mas o deploy via `infra/glue_etl.tf` sempre passa `var.translate_provider`, hoje `"aws"` — serviço de tradução/detecção primário usado em `configuration`; o outro é usado automaticamente como fallback — ver `resolve_translate_fn` em `shared_utils.traducao`) |
| **Leitura** | S3 SOR — JSON bruto por tipo de tabela e ano |
| **Escrita** | S3 SOT — Parquet particionado (ou não) + registro no Glue Catalog |
| **Aciona** | Glue Data Quality (sempre) + Glue Details (apenas para `TABLE_TYPE=discover`, repassando o próprio `TRANSLATE_PROVIDER` deste job — é esse valor, não o `default_arguments` de `glue_details.tf`, que decide o provider do fluxo normal de discover) |
...
| `read_from_sor(bucket, media_type, table_type, year=None, translate_fn=None, s3_bucket_sot=None, table_name=None, detect_fn=None)` | Lê JSON/Parquet da camada SOR; para `discover` adiciona `overview_detected_language` (diagnóstico, via `detect_fn`) e `overview_translated_pt_br` (derivado, sem tradução); para `configuration` adiciona tradução `name_pt` (countries em tv, languages em movie) via `translate_fn`, mais `name_detected_language_en`/`name_detected_language_pt`/`name_translation_attempts` via `detect_fn`. Quando `s3_bucket_sot`/`table_name` são informados (só relevante para `configuration`), lê a tabela já gravada na SOT via `read_existing_configuration` e usa como cache de tradução (`_add_name_pt_countries`/`_add_name_pt_languages`) |
| `_add_translation(df, description, key_column, translate_fn=None, previous_df=None, detect_fn=None)` | Reaproveita `name_pt` de `previous_df` via `reuse_existing_translation` (`shared_utils.traducao`) quando `english_name` não mudou para a mesma `key_column`, antes de traduzir com `max_workers=2` (volumes pequenos, ~250 itens; o paralelismo só sobrepõe o backoff do Google Translate, não é para ganhar vazão). O resto do fluxo (detecção de idioma da fonte e do resultado, cópia direta, tradução via `translate_fn`, teto de tentativas) é responsabilidade de `shared_utils.traducao.resolve_pt_translation` — grava `name_detected_language_en`, `name_detected_language_pt`, `name_pt` e `name_translation_attempts` |
...
| `translate_text_aws(texto, region)` | `shared_utils.traducao_aws` (reexportada em `shared_utils.traducao`) | Traduz texto via AWS Translate; primário no deploy atual (`var.translate_provider="aws"`), sem teto; usado como fallback capado por caracteres só quando o primário é `"google"` |
| `resolve_translate_fn(provider, translate_google=translate_text, translate_aws=translate_text_aws, aws_fallback_max_chars=2_000_000, get_chars_used_this_month=get_translate_chars_used_this_month)` | `shared_utils.traducao` | Resolve `TRANSLATE_PROVIDER` para uma função composta primário+fallback (o teto de `aws_fallback_max_chars` só importa quando `"google"` é o primário); montada uma vez em `main()` e passada a `read_from_sor`, e repassada ao acionar `glue_details` |
...
| `resolve_detect_language_fn(detect_local=detect_language_langdetect, detect_aws=detect_language_aws, aws_fallback_max_chars=6_000, provider=translate_provider)` | `shared_utils.idioma` | Resolve a função composta de detecção, espelhando `provider` de `resolve_translate_fn`: `TRANSLATE_PROVIDER="google"` → local primeiro, AWS Comprehend como fallback capado por caracteres; `"aws"` → Comprehend primeiro (sem cap), local como fallback. Montada uma vez em `main()` (com o mesmo `translate_provider` passado a `resolve_translate_fn`) e passada a `read_from_sor` |
| `add_detected_language_column(df, source_column, target_column, detect_fn=None, only_missing=False)` | `shared_utils.idioma` | Aplica `detect_fn` a cada valor de `source_column`, gravando o idioma detectado em `target_column` — usada diretamente para `overview_detected_language` (discover; a partição é sempre reconstruída do zero, sem `only_missing`) |
...
- **langdetect** — detecção local de idioma para `overview_detected_language` (discover) e `name_detected_language_en`/`name_detected_language_pt` (configuration)
