...
6. **Tradução de sinopses (TMDB pt-BR → cache do S3 → Google Translate → AWS Translate):** primeiro verifica se o TMDB já possui tradução pt-BR (extraída do `append_to_response=translations`). Caso exista, usa diretamente. Caso contrário, verifica se `overview_en` é idêntico ao registro stale lido do S3 durante o cálculo do delta (ver passo 4) — se for, reaproveita `overview_pt` já persistido sem chamar a API de tradução de novo. Só quando não há tradução nativa nem cache aproveitável, para títulos com `original_language` diferente de `pt` e `overview_en` não-vazio, traduz via `deep_translator.GoogleTranslator` com `source="auto"` (detecção automática de idioma; com retry/backoff em `traduzir_texto`, já que o endpoint não-oficial falha esporadicamente sob alto volume). Se o Google Translate falhar (resultado igual ao original), tenta o **AWS Translate** como 3ª camada, limitado a `AWS_TRANSLATE_MAX_PER_RUN` chamadas na execução (0 = desligado, padrão). Grava na coluna `overview_pt`. Para títulos já em `pt` ou com `overview_en` vazio, `overview_pt` fica nulo
6b. **Tradução de keywords:** mesma prioridade de cache do item 6 (reaproveita `keywords_pt` existente quando `keywords` não mudou desde o último processamento) antes de traduzir as keywords temáticas (sempre em inglês na TMDB para idiomas diferentes de `pt`, não suportam localização) para português via Google Translate (com o mesmo fallback AWS Translate acima), gravando na coluna `keywords_pt`. Traduz registros com keywords não-nulas e `original_language` diferente de `pt` — pula conteúdo já em português para não gastar chamadas de tradução à toa
6c. **Tradução de tagline (TMDB pt-BR → cache do S3 → Google Translate → AWS Translate):** primeiro verifica se o TMDB já possui tradução pt-BR da tagline. Caso exista, usa diretamente. Caso contrário, reaproveita `tagline_pt` existente quando `tagline` não mudou desde o último processamento (mesma lógica de cache do item 6). Só então traduz via Google Translate — o idioma de origem é detectado automaticamente (`source="auto"`), o que importa aqui porque a tagline é traduzida para qualquer `original_language` diferente de `pt` — com o mesmo fallback AWS Translate. Grava na coluna `tagline_pt`
...
| | Descrição |
|---|---|
| **Entrada** | Argumentos: `MEDIA_TYPE`, `YEAR`, `END_YEAR`, `DATABASE`, nomes dos buckets e jobs, `FORCE_REFETCH` (opcional, default `false`), `AWS_TRANSLATE_MAX_PER_RUN` (opcional, default `0` — limite de chamadas ao AWS Translate nesta execução) |
...
|---|---|
| `get_parameters_glue()` | Lê e valida os argumentos de execução do job (inclui leitura manual de `FORCE_REFETCH` e `AWS_TRANSLATE_MAX_PER_RUN`, ambos opcionais) |
...
| `_adicionar_traducoes_pt(df, traduzir_fn=None, df_anterior=None)` | Prioriza overview pt-BR do TMDB; em seguida reaproveita `overview_pt` existente via `reaproveitar_traducao_existente` (`shared_utils.traducao`) quando `overview_en` não mudou; só então traduz via `shared_utils.traducao.traduzir_coluna_pendente` usando `traduzir_fn` (Google Translate puro por padrão; `collect_and_write_details` injeta a versão com fallback AWS Translate). Elegibilidade (`original_language != 'pt'` e `overview_en` não-vazio) vem de `shared_utils.traducao.elegivel_overview_pt` |
| `_adicionar_traducoes_keywords_pt(df, traduzir_fn=None, df_anterior=None)` | Reaproveita `keywords_pt` existente via `reaproveitar_traducao_existente` quando `keywords` não mudou; caso contrário traduz para PT (TMDB não localiza keywords) via `shared_utils.traducao.traduzir_coluna_pendente` usando `traduzir_fn`. Elegibilidade (`original_language != 'pt'` e `keywords` não-vazio) vem de `shared_utils.traducao.elegivel_keywords_pt` |
| `_adicionar_traducoes_tagline_pt(df, traduzir_fn=None, df_anterior=None)` | Prioriza tagline pt-BR do TMDB; em seguida reaproveita `tagline_pt` existente via `reaproveitar_traducao_existente` quando `tagline` não mudou; só então traduz via `shared_utils.traducao.traduzir_coluna_pendente` usando `traduzir_fn` (idioma de origem detectado automaticamente). Elegibilidade (`original_language != 'pt'` e `tagline` não-vazio) vem de `shared_utils.traducao.elegivel_tagline_pt` |
...
| `collect_and_write_details(ids, ..., aws_translate_max_calls=0)` | Faz chamadas paralelas e grava tabela de detalhes. Antes de traduzir, lê o S3 uma única vez por partição `year` afetada, separando os registros existentes em `df_existing_delta` (ids do delta atual, usados como cache de tradução) e `df_existing_keep` (ids fora do delta, preservados no merge final) — a mesma leitura alimenta as duas finalidades. Monta o `traduzir_fn` (Google Translate + fallback AWS Translate via `criar_traduzir_fn_com_aws_translate`) uma vez por execução e passa, junto com `df_existing_delta`, às 3 `_adicionar_traducoes_*` |
...
| `traduzir_em_paralelo(valores, traduzir_fn, max_workers)` | `shared_utils.traducao` | Aplica `traduzir_fn` a cada valor em paralelo via `ThreadPoolExecutor`; usada por `traduzir_coluna_pendente` |
| `traduzir_coluna_pendente(df, coluna_fonte, coluna_destino, mask_elegivel, traduzir_fn, max_workers)` | `shared_utils.traducao` | Orquestra a tradução coluna a coluna: pula registros já traduzidos (`coluna_destino` preenchida e diferente de `coluna_fonte`), retenta os que ficaram iguais à fonte (fallback de falha anterior), grava o resultado e devolve a contagem de sucesso. Compartilhada com `scripts/backfill_traducao.py` para evitar que as duas cópias divirjam |
| `reaproveitar_traducao_existente(df, df_anterior, coluna_fonte, coluna_destino, coluna_chave="id")` | `shared_utils.traducao` | Pré-preenche `coluna_destino` do `df` novo com o valor já persistido (`df_anterior`) quando `coluna_fonte` não mudou para o mesmo `coluna_chave`. Não sobrescreve valor já preenchido (prioridade da tradução nativa do TMDB); registros novos sem histórico ou schema antigo sem a coluna não são afetados. O reaproveitamento é reconhecido pela checagem "já traduzido" já existente em `traduzir_coluna_pendente`, sem duplicar lógica. Compartilhada com `glue_etl` (que usa `coluna_chave="iso_3166_1"`/`"iso_639_1"` para a tabela `configuration`) |
| `criar_traduzir_fn_com_aws_translate(traduzir_fn_primario, max_chamadas, region)` | `shared_utils.traducao` | Compõe `traduzir_texto` com fallback via AWS Translate (3ª camada), limitado a `max_chamadas` por execução — `AWS_TRANSLATE_MAX_PER_RUN` |
| `elegivel_overview_pt(df)` / `elegivel_tagline_pt(df)` / `elegivel_keywords_pt(df)` | `shared_utils.traducao` | Masks de candidatos à tradução por campo, compartilhadas com `scripts/backfill_traducao.py` para evitar que as duas cópias divirjam |
...
- **AWS Translate** (via boto3) — 3ª camada de tradução, acionada só quando o Google Translate falha; sem custo de API key/secret (usa a role IAM do job)
