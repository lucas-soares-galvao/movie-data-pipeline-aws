...
| `_traduzir_coluna(df, mask, coluna_fonte, coluna_destino)` | Helper compartilhado pelas três `_adicionar_traducoes_*`: delega a `shared_utils.traducao.traduzir_em_paralelo` a tradução paralela dos valores selecionados por `mask`, gravando o resultado na coluna de destino |
