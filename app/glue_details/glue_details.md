...
| `_adicionar_traducoes_pt(df)` | Prioriza overview pt-BR do TMDB; fallback para Google Translate via `shared_utils.traducao.traduzir_coluna_pendente` (compartilhada com `scripts/backfill_traducao.py`). Elegibilidade (`original_language != 'pt'` e `overview_en` não-vazio) vem de `shared_utils.traducao.elegivel_overview_pt` |
| `_adicionar_traducoes_keywords_pt(df)` | Traduz keywords para PT via Google Translate (TMDB não localiza keywords), via `shared_utils.traducao.traduzir_coluna_pendente`. Elegibilidade (`original_language != 'pt'` e `keywords` não-vazio) vem de `shared_utils.traducao.elegivel_keywords_pt` |
| `_adicionar_traducoes_tagline_pt(df)` | Prioriza tagline pt-BR do TMDB; fallback para Google Translate via `shared_utils.traducao.traduzir_coluna_pendente` (idioma de origem detectado automaticamente). Elegibilidade (`original_language != 'pt'` e `tagline` não-vazio) vem de `shared_utils.traducao.elegivel_tagline_pt` |
