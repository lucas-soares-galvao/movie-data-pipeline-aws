...
6. **Tradução de sinopses (TMDB pt-BR → Google Translate):** primeiro verifica se o TMDB já possui tradução pt-BR (extraída do `append_to_response=translations`). Caso exista, usa diretamente. Caso contrário, para títulos com `original_language='en'` e `overview_en` não-vazio, traduz via `deep_translator.GoogleTranslator` com `source="auto"` (detecção automática de idioma; com retry/backoff em `traduzir_texto`, já que o endpoint não-oficial falha esporadicamente sob alto volume). Grava na coluna `overview_pt`. Para outros idiomas ou `overview_en` vazio, `overview_pt` fica nulo
...
| `_adicionar_traducoes_pt(df)` | Prioriza overview pt-BR do TMDB; fallback para Google Translate. Elegibilidade (`original_language='en'` e `overview_en` não-vazio) vem de `shared_utils.traducao.elegivel_overview_pt`, compartilhada com `scripts/backfill_traducao.py` |
| `_adicionar_traducoes_keywords_pt(df)` | Traduz keywords para PT via Google Translate (TMDB não localiza keywords). Elegibilidade vem de `shared_utils.traducao.elegivel_keywords_pt`, compartilhada com `scripts/backfill_traducao.py` |
| `_adicionar_traducoes_tagline_pt(df)` | Prioriza tagline pt-BR do TMDB; fallback para Google Translate (idioma de origem detectado automaticamente, já que a tagline pode não estar em inglês). Elegibilidade vem de `shared_utils.traducao.elegivel_tagline_pt`, compartilhada com `scripts/backfill_traducao.py` |
