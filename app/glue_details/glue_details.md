...
| `_adicionar_traducoes_pt(df, traduzir_fn=None)` | Prioriza overview pt-BR do TMDB; fallback via `shared_utils.traducao.traduzir_coluna_pendente` usando `traduzir_fn` (Google Translate puro por padrão; `collect_and_write_details` injeta a versão com fallback AWS Translate). Elegibilidade (`original_language != 'pt'` e `overview_en` não-vazio) vem de `shared_utils.traducao.elegivel_overview_pt` |
| `_adicionar_traducoes_keywords_pt(df, traduzir_fn=None)` | Traduz keywords para PT (TMDB não localiza keywords) via `shared_utils.traducao.traduzir_coluna_pendente` usando `traduzir_fn`. Elegibilidade (`original_language != 'pt'` e `keywords` não-vazio) vem de `shared_utils.traducao.elegivel_keywords_pt` |
| `_adicionar_traducoes_tagline_pt(df, traduzir_fn=None)` | Prioriza tagline pt-BR do TMDB; fallback via `shared_utils.traducao.traduzir_coluna_pendente` usando `traduzir_fn` (idioma de origem detectado automaticamente). Elegibilidade (`original_language != 'pt'` e `tagline` não-vazio) vem de `shared_utils.traducao.elegivel_tagline_pt` |
...
| `collect_and_write_details(ids, ..., aws_translate_max_calls=0)` | Faz chamadas paralelas e grava tabela de detalhes; monta o `traduzir_fn` (Google Translate + fallback AWS Translate via `criar_traduzir_fn_com_aws_translate`) uma vez por execução e passa às 3 `_adicionar_traducoes_*` |
