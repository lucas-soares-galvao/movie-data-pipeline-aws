...
| `_traduzir_coluna(df, mask, coluna_fonte, coluna_destino)` | Helper compartilhado pelas três `_adicionar_traducoes_*`: delega a `shared_utils.traducao.traduzir_em_paralelo` a tradução paralela dos valores selecionados por `mask`, gravando o resultado na coluna de destino. Loga em INFO um resumo do tipo "X de Y traduzidos com sucesso (coluna)" ao final, contando como sucesso os casos em que o texto traduzido difere do original |
| `_adicionar_traducoes_pt(df)` | Prioriza overview pt-BR do TMDB; fallback para Google Translate. Elegibilidade (`original_language != 'pt'` e `overview_en` não-vazio) vem de `shared_utils.traducao.elegivel_overview_pt`, compartilhada com `scripts/backfill_traducao.py` |
| `_adicionar_traducoes_keywords_pt(df)` | Traduz keywords para PT via Google Translate (TMDB não localiza keywords). Elegibilidade (`original_language != 'pt'` e `keywords` não-vazio) vem de `shared_utils.traducao.elegivel_keywords_pt`, compartilhada com `scripts/backfill_traducao.py` |
| `_adicionar_traducoes_tagline_pt(df)` | Prioriza tagline pt-BR do TMDB; fallback para Google Translate (idioma de origem detectado automaticamente). Elegibilidade (`original_language != 'pt'` e `tagline` não-vazio) vem de `shared_utils.traducao.elegivel_tagline_pt`, compartilhada com `scripts/backfill_traducao.py` |
...
| `traduzir_em_paralelo(valores, traduzir_fn, max_workers)` | `shared_utils.traducao` | Aplica `traduzir_fn` a cada valor em paralelo via `ThreadPoolExecutor`; usada por `_traduzir_coluna` e compartilhada com `scripts/backfill_traducao.py` |
