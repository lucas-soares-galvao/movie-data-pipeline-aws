...
Liga e desliga a instância Lightsail onde o FilmBot roda — via `terraform apply`/`destroy -target`, não stop/start (o Lightsail cobra a mesma tarifa do bundle em ambos os estados; só destruir a instância reduz custo de verdade). Em prod roda por cron automático (GitHub Actions `schedule`) tanto para ligar quanto para desligar; em dev (bundle `nano_3_0`, mais barato, em `filmbot-dev.lsgalvao.com.br`) o cron só desliga automaticamente todo dia — ligar continua sendo só via disparo manual (`workflow_dispatch`). O IP estático nunca é destruído, então o DNS não precisa ser reconfigurado a cada ciclo.
